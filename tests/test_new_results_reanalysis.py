...
from src.experiments.paper_figures.new_results_reanalysis import _layer2_ux


def test_exact_sign_flip_one_sided_all_positive() -> None:
    values = np.asarray([1.0, 2.0, 3.0])
    assert exact_sign_flip_p(values, alternative="greater") == 1.0 / 8.0
    assert exact_sign_flip_p(-values, alternative="less") == 1.0 / 8.0
