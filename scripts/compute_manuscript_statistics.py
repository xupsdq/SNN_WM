...
TASKS: tuple[TaskSpec, ...] = (
    ...
    TaskSpec(
        ...
        "Before Fig.1D/E seed_1005 is filled, this task is partial with n=19.",
    ),
    TaskSpec(
        "Q04",
        "fig2",
    ...
    TaskSpec(
        ...
        "q11",
        "Within sequence_state/S_final rows, matched cues are paired against mismatched and unseen cues.",
    ),
    TaskSpec(
        "Q12",
        "fig3",
    ...
    TaskSpec(
        ...
        "fig4",
        "C",
        "high-overlap high-similarity pairs show stronger STSP-dependent bias.",
        (
            SourceSpec(
                "data/metrics/panel_c_overlap_matched_comparison.csv",
                ("network_seed", "matched_group_id", "overlap_group", "acc_drop", "pixel_similarity", "dice_overlap"),
            ),
        ),
        "q15",
        "Matched high_overlap-low_overlap acc_drop contrasts are averaged within seed and tested.",
    ),
    TaskSpec(
        "Q16",
        "fig4",
        "D",
        "overlap-preserved support retains more dynamic-state influence than controls.",
        (
            SourceSpec(
        ...
        "q16",
        "Precomputed seed-level overlap-control contrasts are tested vs 0.",
    ),
    TaskSpec(
        "Q17",
        "fig4",
    ...
    TaskSpec(
        ...
        "fig5",
        "C",
        "dynamic winners show pre-spike voltage gain over static reference.",
        (
            SourceSpec(
                "data/metrics/panel_c_winner_loser_event_metrics.csv",
                ("network_seed", "winner_group", "winner_pre_spike_delta_v_mean", "winner_pre_spike_boost"),
            ),
        ),
        "q21",
        "Event-level voltage gains are averaged within seed and tested vs 0.",
    ),
    TaskSpec(
        "Q22",
        "fig5",
        ...
        (
            SourceSpec(
                ...
                (
                    ...
                    "attenuate_minus_reset_transition_mass",
                ),
            ),
        ),
        "q22",
        "Precomputed seed-level perturbation contrasts are tested vs 0.",
    ),
    TaskSpec(
        "Q23",
        "fig6",
    ...
    TaskSpec(
        ...
        "q26",
        "Seed-level interaction_delta is tested vs 0 by early response window.",
    ),
    TaskSpec(
        "Q27",
        "fig6",
        ...
        "q27",
        "Seed-level paired loss difference high-STSP-overlap minus matched-removal is tested vs 0.",
    ),
)
...
def _auc_rows(
    ...
    *,
    ...
    for key, sub in work.groupby(["network_seed", *group_cols, factor_col], dropna=False):
        ...
        if len(np.unique(x)) < 2:
            continue
        auc_records.append({"network_seed": seed, **group_values, factor_col: condition, "auc": float(np.trapezoid(y, x))})
    if not auc_records:
        return []
    return _paired_rows(
        task,
...
def _calc_q15(root: Path, task: TaskSpec, seeds: Sequence[int]) -> list[dict[str, str]]:
    ...
    return _paired_rows(
        ...
        df,
        "acc_drop",
        factor_col="overlap_group",
        pairs=(("high_overlap", "low_overlap"),),
        metric="high_minus_low_overlap_acc_drop",
    )
...
def _calc_q16(root: Path, task: TaskSpec, seeds: Sequence[int]) -> list[dict[str, str]]:
    ...
    return _column_contrast_rows(
        ...
        df,
        (
            ("overlap_minus_nonoverlap_DPI", "DPI_overlap_minus_nonoverlap"),
            ("overlap_minus_random_DPI", "DPI_overlap_minus_random"),
            ("overlap_minus_nonoverlap_recovery", "recovery_overlap_minus_nonoverlap"),
            ("overlap_minus_random_recovery", "recovery_overlap_minus_random"),
        ),
    )
...
def _calc_q21(root: Path, task: TaskSpec, seeds: Sequence[int]) -> list[dict[str, str]]:
    df, _present = _read_csvs(root, task, task.sources[0].path, seeds)
    rows = _one_sample_rows(task, df, "winner_pre_spike_delta_v_mean", metric="winner_pre_spike_delta_v_mean")
    rows.extend(_one_sample_rows(task, df, "winner_pre_spike_boost", metric="winner_pre_spike_boost"))
    return rows


def _calc_q22(root: Path, task: TaskSpec, seeds: Sequence[int]) -> list[dict[str, str]]:
    df, _present = _read_csvs(root, task, task.sources[0].path, seeds)
    return _column_contrast_rows(
        task,
        df,
        (
            ("dynamic_minus_attenuate_transition_mass", "dynamic_minus_attenuate_transition_mass"),
            ("dynamic_minus_reset_transition_mass", "dynamic_minus_reset_transition_mass"),
            ("attenuate_minus_reset_transition_mass", "attenuate_minus_reset_transition_mass"),
        ),
    )
...
def _calc_q26(root: Path, task: TaskSpec, seeds: Sequence[int]) -> list[dict[str, str]]:
    df, _present = _read_csvs(root, task, task.sources[0].path, seeds)
    rows = _one_sample_rows(task, df, "interaction_delta", metric="interaction_delta", group_cols=("early_window_ms",))
    rows.extend(_one_sample_rows(task, df, "stsp_effect_with_overlap", metric="stsp_effect_with_overlap", group_cols=("early_window_ms",)))
    rows.extend(_one_sample_rows(task, df, "stsp_effect_without_overlap", metric="stsp_effect_without_overlap", group_cols=("early_window_ms",)))
    return rows
