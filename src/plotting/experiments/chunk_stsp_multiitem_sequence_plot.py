...
from src.plotting.experiments.chunk_stsp_multiitem_sequence_plot_lib import load_plot_bundle, render_figures


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot-only entrypoint for chunk_stsp_multiitem_sequence.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"[Output] Generated {len(figure_paths)} figure groups for chunk_stsp_multiitem_sequence -> {output_dir}",
        flush=True,
    )
    return 0
