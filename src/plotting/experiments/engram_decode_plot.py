...
from src.plotting.experiments.engram_decode_plot_lib import load_plot_bundle, render_figures


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot-only entrypoint for engram_decode.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"[Output] Generated {len(figure_paths)} figure groups for engram_decode -> {output_dir}", flush=True)
    return 0
