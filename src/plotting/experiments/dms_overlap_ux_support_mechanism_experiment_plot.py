...
from src.plotting.experiments.dms_overlap_ux_support_mechanism_experiment_plot_lib import load_plot_bundle, render_panels


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot-only entrypoint for dms_overlap_ux_support_mechanism_experiment.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    panel_paths = render_panels(bundle, figures_dir=output_dir)
    print(
        f"[Output] Generated {len(panel_paths)} panels for dms_overlap_ux_support_mechanism_experiment -> {output_dir}",
        flush=True,
    )
    return 0
