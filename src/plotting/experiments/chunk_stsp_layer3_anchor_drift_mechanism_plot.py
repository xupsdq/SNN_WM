...
from src.plotting.experiments.chunk_stsp_layer3_anchor_drift_mechanism_plot_lib import load_plot_bundle, render_panels


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot-only entrypoint for chunk_stsp_layer3_anchor_drift_mechanism.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"[Output] Generated {len(panel_paths)} panels for chunk_stsp_layer3_anchor_drift_mechanism -> {output_dir}",
        flush=True,
    )
    return 0
