...
from src.plotting.experiments.chunk_step2_fused_state_experiment_plot_lib import load_plot_bundle, render_retained_panels


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot-only entrypoint for chunk_step2_fused_state_experiment.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    outputs = render_retained_panels(bundle, figures_dir=output_dir)
    print(
        f"[Output] Generated {len(outputs)} retained panel groups for chunk_step2_fused_state_experiment -> {output_dir}",
        flush=True,
    )
    return 0
