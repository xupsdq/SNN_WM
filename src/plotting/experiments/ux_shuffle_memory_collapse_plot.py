...
from src.plotting.experiments.ux_shuffle_memory_collapse_plot_lib import load_plot_bundle, render_figures


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot-only entrypoint for ux_shuffle_memory_collapse.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"[Output] Generated {len(figure_paths)} figure groups for ux_shuffle_memory_collapse -> {output_dir}", flush=True)
    return 0
