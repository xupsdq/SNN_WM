...
from src.plotting.experiments.l3_accumulator_mechanism_experiment_plot_lib import load_plot_bundle, render_figures


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot-only entrypoint for l3_accumulator_mechanism_experiment.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"[Output] Generated {len(figure_paths)} retained figure groups for l3_accumulator_mechanism_experiment -> {output_dir}",
        flush=True,
    )
    return 0
