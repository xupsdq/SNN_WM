...
from src.plotting.experiments.overlap_causal_input_perturbation_experiment_plot_lib import load_plot_bundle, render_figures


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot-only entrypoint for overlap_causal_input_perturbation_experiment.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    figure_paths = render_figures(bundle, figures_dir=output_dir)
    print(
        f"[Output] Generated {len(figure_paths)} figure groups for overlap_causal_input_perturbation_experiment -> {output_dir}",
        flush=True,
    )
    return 0
