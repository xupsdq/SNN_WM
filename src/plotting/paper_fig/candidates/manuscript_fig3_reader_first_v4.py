...
CANDIDATE_VERSION = "manuscript_fig3_reader_first_v4"
...
SPEC_PATH = (
    ...
    / "specs"
    / "manuscript_fig3_reader_first_v4.json"
)
...
def _plot_joint_mean(
    axis: plt.Axes,
    payload: Mapping[str, Any],
    panel_spec: Mapping[str, Any],
) -> None:
    x_stat = payload["update_stat_rows"]["same_B_common_update_cosine"]
    y_stat = payload["update_stat_rows"][
        "processing_residual_gamma_norm_ratio"
    ]
    x_mean = float(x_stat["estimate"])
    y_mean = float(y_stat["estimate"])
    x_low, x_high = float(x_stat["ci95_low"]), float(x_stat["ci95_high"])
    y_low, y_high = float(y_stat["ci95_low"]), float(y_stat["ci95_high"])
    x_criterion = float(panel_spec["x_criterion"])
    y_criterion = float(panel_spec["y_criterion"])
    if not math.isclose(x_criterion, float(x_stat["null_value"]), abs_tol=1e-12):
        raise ValueError("Fig.3c x criterion does not match frozen statistics")
    if not math.isclose(y_criterion, float(y_stat["null_value"]), abs_tol=1e-12):
        raise ValueError("Fig.3c y criterion does not match frozen statistics")

    x_limits = [float(value) for value in panel_spec["x_limits"]]
    y_limits = [float(value) for value in panel_spec["y_limits"]]
    if not x_limits[0] <= x_mean <= x_limits[1]:
        raise ValueError("Fig.3c x mean is outside the display limits")
    if not y_limits[0] <= y_mean <= y_limits[1]:
        raise ValueError("Fig.3c y mean is outside the display limits")

    projection_color = _resolve_color(str(panel_spec["projection_color"]))
    mean_color = _resolve_color(str(panel_spec["mean_color"]))
    projection_style = (0, (3.0, 2.3))
    axis.plot(
        [x_criterion, x_mean],
        [y_mean, y_mean],
        color=projection_color,
        linewidth=0.75,
        linestyle=projection_style,
        zorder=1,
    )
    axis.plot(
        [x_mean, x_mean],
    ...
    x_ticks = [float(value) for value in panel_spec["x_ticks"]]
    y_ticks = [float(value) for value in panel_spec["y_ticks"]]
    criterion_label = str(panel_spec["criterion_label"])
    axis.set_xlim(*x_limits)
    axis.set_ylim(*y_limits)
    axis.set_xticks(x_ticks)
    axis.set_yticks(y_ticks)
    axis.set_xticklabels([f"{value:g}" for value in x_ticks])
    axis.set_yticklabels([f"{value:g}" for value in y_ticks])
    x_span = x_limits[1] - x_limits[0]
    y_span = y_limits[1] - y_limits[0]
    axis.text(
        x_criterion + 0.08 * x_span,
        y_criterion + 0.025 * y_span,
        criterion_label,
        ha="left",
        va="bottom",
        color=NEUTRAL_MID,
    ...
    axis.text(
        x_criterion + 0.018 * x_span,
        y_criterion + 0.10 * y_span,
        criterion_label,
        ha="left",
        va="bottom",
        rotation=90,
...
def _candidate_tables(payload: Mapping[str, Any]) -> dict[str, pd.DataFrame]:
    ...
    common = payload["update_stat_rows"]["same_B_common_update_cosine"]
    history = payload["update_stat_rows"][
        "processing_residual_gamma_norm_ratio"
    ]
        ...
        [
            {
                "candidate_figure": "Fig.3",
                "candidate_panel": "c",
                "display": "two_dimensional_network_mean_coordinate",
                "independent_unit": "network_seed",
                ...
                "network_weight": 1.0 / len(EXPECTED_SEEDS),
                "mean_update_similarity": float(common["estimate"]),
                "update_ci95_low": float(common["ci95_low"]),
                "update_ci95_high": float(common["ci95_high"]),
                "update_criterion": float(common["null_value"]),
                "mean_history_effect": float(history["estimate"]),
                "history_ci95_low": float(history["ci95_low"]),
                "history_ci95_high": float(history["ci95_high"]),
                "history_criterion": float(history["null_value"]),
    ...
    return {
        ...
        "panel_c_mean_summary": mean_summary,
        **metric_tables,
    }
    ...
    rows.append(
        {
            ...
            "independent_unit": "network_seed",
            "included_networks": "1000-1019",
            "mapping": "500 valid one-step comparisons per network → one within-network x/y mean → equal-weight two-dimensional mean across 20 networks; artwork shows frozen horizontal and vertical 95% CIs",
        }
    )
...
def _render_figure(
    ...
    with plt.rc_context(
        {
            **VECTOR_TEXT_RCPARAMS,
            "svg.hashsalt": "net_torch_manuscript_fig3_reader_first_v4",
        ...
        for panel_id, panel_spec in spec["panels"].items():
            ...
            elif chart == "joint_mean_projection":
                _plot_joint_mean(axis, payload, panel_spec)
...
def build_candidate(
    *,
    ...
    summary = {
        ...
        "joint_mean_coordinate": {
            "comparison_rows": int(len(payload["joint"])),
            "comparisons_per_network": 500,
            "rows_above_both_criteria": int(payload["both_above_criteria"]),
            "network_weighting": "equal 1/20 weight",
            "display": "single two-dimensional mean point with frozen x/y 95% CIs and axis projections",
        },
        "outputs": {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render the formal reader-first manuscript Fig.3."
    )
    parser.add_argument(
        "--parent-dir",
        default=(
    ...
    parser.add_argument(
        "--output-dir",
        default="results/paper_figures/outputs/provenance/fig3",
    )
