...
CANDIDATE_VERSION = "manuscript_fig4_reader_first_v6"
DISPLAY_NAME = "Fig.4"
...
SPEC_PATH = (
    ...
    / "specs"
    / "manuscript_fig4_reader_first_v6.json"
)
...
def _load_sources(
    ...
    # ------------------------------------------------------------- panel g
    # V4 intentionally has no raster input dependency: the schematic uses
    # an abstract current Input node and inherited STSP state.
...
# Distinct next-layer successor template; never reuse DOWNSTREAM_NODES.
SUCCESSOR_NODES = [
    (42, 38, 9), (96, 48, 7), (151, 36, 8), (68, 70, 6),
    (120, 65, 10), (162, 83, 7), (34, 106, 8), (86, 119, 7),
    (139, 108, 8), (160, 140, 6), (55, 150, 7), (110, 145, 9),
]


def _build_g_asset_svg(payload: Mapping[str, Any], spec: Mapping[str, Any]) -> bytes:
    """Build the centred, single-input, four-stage mechanism strip for panel g."""
    width = 1500
    height = 430
    box_y = 78.0
    box_w = 280.0
    box_h = 240.0
    main_y = box_y + box_h / 2.0
    label_y = 360.0
    stage_centres = {
        "inherited": 170.0,
        "selected": 550.0,
    ...
    field_scale_y = 0.90
    field_origin_x = {
        "inherited": box_x["inherited"] + 18.0,
        "selected": box_x["selected"] + 0.0,
        "downstream": box_x["downstream"] + 7.0,
        "successor": box_x["successor"] + 10.0,
    }
    field_origin_y = {
        "inherited": 115.0,
        "selected": 126.0,
    ...
    input_centre = (box_x["inherited"] + box_w + box_x["selected"]) / 2.0
    arrow_color = "#666666"
    inherited_color = "#009E73"
    firing_color = "#0072B2"
    successor_color = "#CC79A7"
    text_color = "#222222"

    def field_position(stage: str, px: float, py: float) -> tuple[float, float]:
        return (
            field_origin_x[stage] + px * field_scale_x,
            field_origin_y[stage] + py * field_scale_y,
        )
    ...
    def dot(
        ...
        r = max(7.0, radius * 1.05)
        return (
            f'<circle class="state-dot" data-stage="{stage}" cx="{x:.2f}" '
            f'cy="{y:.2f}" r="{r:.2f}" fill="{color}" '
            f'fill-opacity="{opacity:g}" stroke="#FFFFFF" stroke-width="2.6"/>'
        )

    def tick(stage: str, px: float, py: float) -> str:
        x, y = field_position(stage, px, py)
        half = 13.0
        return (
            f'<line class="firing-tick" data-stage="{stage}" x1="{x:.2f}" '
            f'y1="{y - half:.2f}" x2="{x:.2f}" y2="{y + half:.2f}" '
            f'stroke="{firing_color}" stroke-width="5.8" stroke-linecap="round"/>'
        )
    ...
    # Two state colours distinguish the inherited layer from its successor;
    # all firing marks use one navy colour.
    parts.append(
        f'<g font-family="Arial, \'DejaVu Sans\', sans-serif" font-size="32" fill="{text_color}">'
        f'<circle cx="1000" cy="29" r="10" fill="{inherited_color}"/>'
        '<text x="1022" y="39" text-anchor="start">inherited</text>'
        f'<circle cx="1180" cy="29" r="10" fill="{successor_color}"/>'
        '<text x="1202" y="39" text-anchor="start">successor</text>'
        f'<line x1="1370" y1="15" x2="1370" y2="43" stroke="{firing_color}" '
        'stroke-width="5.8" stroke-linecap="round"/>'
        '<text x="1392" y="39" text-anchor="start">firing</text>'
        '</g>'
    )

    stages = (
        ("inherited", "Inherited STSP", "upstream", inherited_color),
        ("selected", "Selected firing", "upstream", firing_color),
        ("downstream", "Downstream firing", "downstream", firing_color),
        ("successor", "Successor STSP", "successor", successor_color),
    )
    for stage, title, template, edge in stages:
        ...
        parts.append(
            ...
            f'y="{label_y:.1f}" text-anchor="middle" font-family="Arial, \'DejaVu Sans\', sans-serif" '
            f'font-size="32" font-weight="700" fill="{edge}">{title}</text>'
        )

    # Input is a label whose downward arrow enters the first-to-second
    # transition; it is not a fifth node.
    parts.append(
        f'<text class="input-label" x="{input_centre:.1f}" y="38" '
        f'text-anchor="middle" font-family="Arial, \'DejaVu Sans\', sans-serif" '
        f'font-size="32" font-weight="700" fill="{text_color}">Input</text>'
    )
    first_gap_start = box_x["inherited"] + box_w
    first_gap_end = box_x["selected"]
    parts.append(
        f'<path class="input-arrow" d="M{input_centre:.1f} 44 V{main_y - 13:.1f}" '
        f'fill="none" stroke="{arrow_color}" stroke-width="5.2" marker-end="url(#arrow-neutral)"/>'
    )
    for start_x, end_x in (
        (first_gap_start, first_gap_end),
        (box_x["selected"] + box_w, box_x["downstream"]),
        (box_x["downstream"] + box_w, box_x["successor"]),
    ):
        parts.append(
            f'<path class="causal-arrow" d="M{start_x:.1f} {main_y:.1f} H{end_x:.1f}" '
            f'fill="none" stroke="{arrow_color}" stroke-width="5.2" marker-end="url(#arrow-neutral)"/>'
        )

    # One glyph class per field.  Successor uses a distinct next-layer map.
    for node in UPSTREAM_NODES:
        parts.append(dot("inherited", node, inherited_color, 1.0))
    for px, py in SELECTED_NODES:
        parts.append(tick("selected", px, py))
    for node in DOWNSTREAM_NODES:
        parts.append(tick("downstream", node[0], node[1]))
    for node_index, node in enumerate(SUCCESSOR_NODES):
        opacity = 0.72 if node_index in (3, 7, 10) else 1.0
        parts.append(dot("successor", node, successor_color, opacity))
...
def _export_pdf_and_png(
    ...
    with tempfile.TemporaryDirectory(
        ...
        writer.metadata = {
            "/Title": "manuscript_fig4_reader_first_v6",
...
def _render_figure(
    ...
    with plt.rc_context(
        {
            **VECTOR_TEXT_RCPARAMS,
            "svg.hashsalt": "net_torch_manuscript_fig4_reader_first_v6",
...
def _render_qa(
    ...
    checks = {
        ...
        "svg_no_raster_inputs": svg_image_count == 0,
    ...
    return {
        ...
        "raster_note": "panel g intentionally contains no raster image elements; Input is an abstract current-input node",
        "pdf": {
            "path": str(pdf),
    ...
    rows.append(
        {
            ...
            "parent_source_scope": "generated candidate asset (deterministic from the v4 schematic grammar)",
            ...
            "mapping": "single left-to-right causal chain; Input is a label and downward arrow entering the midpoint of the inherited-to-selected horizontal arrow; inherited STSP uses teal dots, successor STSP uses magenta dots, and all firing ticks use one navy colour; successor uses a distinct next-layer node template",
        }
    )
    return pd.DataFrame(rows)
...
def build_candidate(
    *,
    ...
    parent_before = pd.concat(
        [bundle_before, overlap_before, swap_before],
        ignore_index=True,
    )
        ...
        [
            {
                ...
                "semantics": "single left-to-right causal chain; Input label and downward arrow enter the midpoint of the first transition; inherited STSP teal dots, successor STSP magenta dots, and one uniform navy firing colour; successor uses a distinct next-layer template",
    ...
    parent_after = pd.concat(
        [bundle_after, overlap_after, swap_after],
        ignore_index=True,
    )
    ...
    summary = {
        ...
        "panel_g_audit": {
            "asset_sha256": _sha256(g_asset_path),
            "topology": "single left-to-right chain; Input is a label and downward arrow entering the midpoint of the inherited-to-selected transition",
            "stage_grammar": "inherited STSP (teal circles) -> selected firing (uniform navy ticks) -> downstream firing (uniform navy ticks) -> successor STSP (magenta circles); Input enters the first transition",
            "alignment": "all four fields share one horizontal centreline; glyph bounds are centred within each field; Input arrow targets the midpoint of the first transition; successor uses a distinct next-layer template",
            "raster_inputs": 0,
        },
        "outputs": {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render the formal reader-first manuscript Fig.4."
    )
    parser.add_argument(
        "--parent-dir",
        default=(
    ...
    parser.add_argument(
        "--output-dir",
        default="results/paper_figures/outputs/provenance/fig4",
    )
