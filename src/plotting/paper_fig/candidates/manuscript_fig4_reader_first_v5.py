...
CANDIDATE_VERSION = "manuscript_fig4_reader_first_v5"
DISPLAY_NAME = "Fig.4"
...
SPEC_PATH = (
    ...
    / "specs"
    / "manuscript_fig4_reader_first_v5.json"
)
...
def _build_g_asset_svg(payload: Mapping[str, Any], spec: Mapping[str, Any]) -> bytes:
    ...
    input_w = 90.0
    input_h = 46.0
    input_x = input_centre - input_w / 2.0
    input_y = main_y - input_h / 2.0
    arrow_color = "#666666"
    ...
    parts.append(
        ...
        f'<circle cx="1030" cy="29" r="10" fill="{inherited_color}"/>'
        '<text x="1052" y="39" text-anchor="start">inherited</text>'
        f'<circle cx="1190" cy="29" r="10" fill="{successor_color}"/>'
        '<text x="1212" y="39" text-anchor="start">successor</text>'
        f'<line x1="1350" y1="15" x2="1350" y2="43" stroke="{firing_color}" '
        'stroke-width="5.8" stroke-linecap="round"/>'
        '<text x="1372" y="39" text-anchor="start">firing</text>'
        '</g>'
    )
    ...
    # Input is a small intermediate node between the first and second stages.
    parts.append(
        f'<rect class="input-node" x="{input_x:.1f}" y="{input_y:.1f}" '
        f'width="{input_w:.1f}" height="{input_h:.1f}" rx="12" fill="#FFFFFF" '
        f'stroke="{arrow_color}" stroke-width="2.6"/>'
    )
    parts.append(
        f'<text class="input-label" x="{input_centre:.1f}" y="{main_y + 11:.1f}" '
    ...
    for start_x, end_x in (
        (box_x["inherited"] + box_w, input_x),
        (input_x + input_w, box_x["selected"]),
...
def _export_pdf_and_png(
    ...
    with tempfile.TemporaryDirectory(
        ...
        writer.metadata = {
            "/Title": "manuscript_fig4_reader_first_v5",
...
def _render_figure(
    ...
    with plt.rc_context(
        {
            **VECTOR_TEXT_RCPARAMS,
            "svg.hashsalt": "net_torch_manuscript_fig4_reader_first_v5",
    ...
    rows.append(
        {
            ...
            "mapping": "single left-to-right causal chain; Input is a compact intermediate node between inherited STSP and selected firing; inherited STSP uses teal dots, successor STSP uses magenta dots, and all firing ticks use one navy colour; successor uses a distinct next-layer node template",
        }
    )
    return pd.DataFrame(rows)
...
def build_candidate(
    *,
        ...
        [
            {
                ...
                "semantics": "single left-to-right causal chain; compact Input node between the first two stages; inherited STSP teal dots, successor STSP magenta dots, and one uniform navy firing colour; successor uses a distinct next-layer template",
    ...
    summary = {
        ...
        "panel_g_audit": {
            "asset_sha256": _sha256(g_asset_path),
            "topology": "single left-to-right chain with a compact Input node inserted between inherited STSP and selected firing",
            "stage_grammar": "inherited STSP (teal circles) -> Input -> selected firing (uniform navy ticks) -> downstream firing (uniform navy ticks) -> successor STSP (magenta circles)",
            "alignment": "all four fields and Input share one horizontal centreline; glyph bounds are centred within each field; successor uses a distinct next-layer template",
            "raster_inputs": 0,
        },
        "outputs": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/paper_figure_candidates/manuscript_fig4_reader_first_v5",
    )
