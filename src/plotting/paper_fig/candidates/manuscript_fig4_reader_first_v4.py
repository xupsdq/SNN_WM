...
CANDIDATE_VERSION = "manuscript_fig4_reader_first_v4"
DISPLAY_NAME = "Fig.4"
...
SPEC_PATH = (
    ...
    / "specs"
    / "manuscript_fig4_reader_first_v4.json"
)
...
def _build_g_asset_svg(payload: Mapping[str, Any], spec: Mapping[str, Any]) -> bytes:
    """Build the compact, single-input, four-stage mechanism strip for panel g."""
    width = 1500
    height = 430
    box_y = 78.0
    box_w = 280.0
    ...
    def field_position(stage: str, px: float, py: float) -> tuple[float, float]:
        return (
            box_x[stage] + 38.0 + px * field_scale_x,
            field_origin_y + py * field_scale_y,
        )
    ...
    def tick(stage: str, px: float, py: float, color: str) -> str:
        ...
        return (
            ...
            f'stroke="{color}" stroke-width="5.8" stroke-linecap="round"/>'
        )
    ...
    # One legend, separated from the stage labels.
    parts.append(
        f'<g font-family="Arial, \'DejaVu Sans\', sans-serif" font-size="32" fill="{text_color}">'
        f'<circle cx="1110" cy="29" r="10" fill="{state_color}"/>'
        '<text x="1132" y="39" text-anchor="start">STSP state</text>'
        f'<line x1="1322" y1="15" x2="1322" y2="43" stroke="{selected_color}" '
        'stroke-width="5.8" stroke-linecap="round"/>'
        '<text x="1344" y="39" text-anchor="start">firing</text>'
        '</g>'
    )
    parts.append(
        f'<text class="input-label" x="{stage_centres["selected"]:.1f}" y="38" '
    ...
    stages = (
        ...
        ("successor", "Successor STSP", "successor", state_color),
    )
    for stage, title, template, edge in stages:
        ...
        # Labels intentionally sit below, not inside, the enlarged fields.
        parts.append(
    ...
    # One vertical Input arrow; every stage-to-stage arrow is horizontal.
    parts.append(
        f'<path class="input-arrow" d="M{stage_centres["selected"]:.1f} 44 V74" '
        f'fill="none" stroke="{arrow_color}" stroke-width="5.2" marker-end="url(#arrow-neutral)"/>'
    )
    for left, right in (("inherited", "selected"), ("selected", "downstream"), ("downstream", "successor")):
        parts.append(
            f'<path class="causal-arrow" d="M{box_x[left] + box_w:.1f} {main_y:.1f} '
            f'H{box_x[right]:.1f}" fill="none" stroke="{arrow_color}" stroke-width="5.2" '
            f'marker-end="url(#arrow-neutral)"/>'
        )

    # One glyph class per field.  The successor field deliberately uses a
    # distinct next-layer template rather than reusing DOWNSTREAM_NODES.
...
def _export_pdf_and_png(
    ...
    with tempfile.TemporaryDirectory(
        ...
        writer.metadata = {
            "/Title": "manuscript_fig4_reader_first_v4",
...
def _render_figure(
    ...
    with plt.rc_context(
        {
            **VECTOR_TEXT_RCPARAMS,
            "svg.hashsalt": "net_torch_manuscript_fig4_reader_first_v4",
    ...
    rows.append(
        {
            ...
            "mapping": "single left-to-right causal chain; one abstract Input node enters selected firing from above; inherited and successor STSP fields contain teal state dots only; selected and downstream firing fields contain vertical spike ticks only; successor uses a distinct next-layer node template",
        }
    )
    return pd.DataFrame(rows)
...
def build_candidate(
    *,
        ...
        [
            {
                ...
                "semantics": "single left-to-right causal chain; one abstract Input node and one downward arrow; STSP fields use teal circles only and firing fields use vertical ticks only; successor uses a distinct next-layer template",
    ...
    summary = {
        ...
        "panel_g_audit": {
            "asset_sha256": _sha256(g_asset_path),
            "topology": "single left-to-right chain with one abstract Input node and one vertical arrow into selected firing",
            ...
            "alignment": "all four stage fields share the same baseline and outer geometry; successor uses a distinct next-layer template",
            "raster_inputs": 0,
        },
        "outputs": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/paper_figure_candidates/manuscript_fig4_reader_first_v4",
    )
