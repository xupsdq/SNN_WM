...
CANDIDATE_VERSION = "manuscript_fig4_reader_first_v3"
DISPLAY_NAME = "Fig.4"
...
SPEC_PATH = (
    ...
    / "specs"
    / "manuscript_fig4_reader_first_v3.json"
)
...
def _load_sources(
    ...
    stimuli = reader.read_csv(
        str(spec["panels"]["g"]["source"]),
        "Fig.4g real previous/current input pixels",
        source_scope="stimulus_bundle",
    )
    ...
    missing = sorted(required_stimulus_columns - set(stimuli.columns))
    if missing:
        raise ValueError(f"Fig.4g stimulus CSV is missing columns: {missing}")
    stimulus_meta: dict[str, dict[str, Any]] = {}
    images: dict[str, np.ndarray] = {}
    for key, expected in spec["stimulus_metadata"].items():
        role = str(expected["stimulus_role"])
        rows = stimuli.loc[stimuli["stimulus_role"].astype(str).eq(role)].copy()
        if len(rows) != 28 * 28:
            raise ValueError(f"Fig.4g stimulus {role} must contain 784 persisted pixels")
        x = pd.to_numeric(rows["pixel_x"], errors="raise").astype(int).to_numpy()
        y = pd.to_numeric(rows["pixel_y"], errors="raise").astype(int).to_numpy()
        values = pd.to_numeric(rows["normalized_intensity"], errors="raise").to_numpy(dtype=float)
        if (
            np.any(x < 0)
            or np.any(x >= 28)
            or np.any(y < 0)
            ...
            raise ValueError(f"Fig.4g stimulus {role} has invalid persisted pixels")
        image = np.zeros((28, 28), dtype=float)
        image[y, x] = values
        images[role] = image
        image_ids = set(pd.to_numeric(rows["image_id"], errors="raise").astype(int))
        labels = set(pd.to_numeric(rows["label"], errors="raise").astype(int))
        hashes = set(rows["image_sha256"].astype(str))
        occurrences = set(pd.to_numeric(rows["render_occurrences"], errors="raise").astype(int))
        if image_ids != {int(expected["image_id"])}:
            raise ValueError(f"Fig.4g stimulus {role} image_id mismatch: {image_ids}")
        if labels != {int(expected["label"])}:
            raise ValueError(f"Fig.4g stimulus {role} label mismatch: {labels}")
        if hashes != {str(expected["image_sha256"])}:
            raise ValueError(f"Fig.4g stimulus {role} image_sha256 mismatch: {hashes}")
        stimulus_meta[role] = {
            "stimulus_role": role,
            "image_id": int(expected["image_id"]),
            "label": int(expected["label"]),
            "image_sha256": str(expected["image_sha256"]),
            "render_occurrences": sorted(occurrences),
            "n_pixels": len(rows),
            "reader_label": "Previous input" if role == "A" else "Current input",
        }
    if stimulus_meta["B"]["render_occurrences"] != [2]:
        raise ValueError("Fig.4g current input B is not registered for two render occurrences")
    stimulus_frame = stimuli.copy()
    stimulus_frame.insert(0, "candidate_figure", DISPLAY_NAME)
    stimulus_frame.insert(0, "candidate_panel", "g")
    ...
    return {
        ...
        "stimuli": stimulus_frame,
        "stimulus_images": images,
...
UPSTREAM_NODES = [
    (35, 48, 10), (90, 30, 8), (142, 51, 7), (58, 82, 6), (108, 66, 11),
    (151, 94, 8), (31, 124, 9), (78, 116, 8), (122, 130, 7), (154, 148, 8),
    (54, 153, 6), (100, 97, 9),
]
SELECTED_NODES = [(90, 30), (108, 66), (122, 130)]
DOWNSTREAM_NODES = [
    (43, 62, 10), (98, 44, 8), (150, 65, 7), (66, 96, 6), (116, 80, 11),
    (159, 108, 8), (39, 138, 9), (86, 130, 8), (130, 144, 7), (162, 162, 8),
    (62, 167, 6), (108, 111, 9),
]
DOWNSTREAM_CYAN = {(116, 80), (159, 108), (86, 130), (62, 167)}


def _raster_png_base64(image: np.ndarray, size: int = 104) -> str:
    """Render a persisted 28x28 intensity matrix as a black-background
    grayscale PNG (0 = background, 1 = stroke) using nearest-neighbour
    upscaling, matching the Fig.3 stimulus presentation."""
    gray = np.clip(image, 0.0, 1.0)
    pixels = (gray * 255.0).astype(np.uint8)
    png = Image.fromarray(pixels, mode="L")
...
def _build_g_asset_svg(payload: Mapping[str, Any], spec: Mapping[str, Any]) -> bytes:
    """Build the reader-first single-baseline mechanism strip for panel g.

    The four stage fields are one horizontal causal chain.  The two persisted
    MNIST rasters enter from above; the only merge is the current-input plus
    inherited-state junction before selected firing.  Each field contains one
    glyph class only: circles for STSP state or vertical ticks for firing.
    """
    images = payload["stimulus_images"]
    role_meta = payload["stimulus_meta"]
    a_b64 = _raster_png_base64(images["A"], size=112)
    b_b64 = _raster_png_base64(images["B"], size=112)

    # The asset is rendered into the full-width 150 x 43 mm g plot slot.
    # Coordinates are deliberately simple so the final-size geometry is easy
    # to audit: all four stage boxes share one y, width, height, and centreline.
    width = 1500
    height = 430
    box_y = 220.0
    box_w = 280.0
    ...
    box_x = {name: cx - box_w / 2.0 for name, cx in stage_centres.items()}
    title_y = box_y + 34.0
    field_origin = (box_x["inherited"] + 42.0, box_y + 52.0)
    field_scale_x = 1.18
    field_scale_y = 0.60
    input_size = 112.0
    input_y = 58.0
    input_a_x = stage_centres["inherited"] - input_size / 2.0
    merge_x = 388.0
    input_b_x = merge_x - input_size / 2.0
    merge_y = 305.0
    arrow_color = "#666666"
    state_color = "#009E73"
    selected_color = "#0072B2"
    downstream_color = "#56B4E9"
    text_color = "#222222"
    border_color = "#D9D9D9"

    def field_position(stage: str, px: float, py: float) -> tuple[float, float]:
        origin_x = box_x[stage] + 42.0
        origin_y = box_y + 52.0
        return origin_x + px * field_scale_x, origin_y + py * field_scale_y

    def dot(stage: str, node: tuple[float, float, float], opacity: float = 1.0) -> str:
        px, py, radius = node
        x, y = field_position(stage, px, py)
        r = max(6.0, radius * 0.82)
        return (
            f'<circle class="state-dot" data-stage="{stage}" cx="{x:.2f}" '
            f'cy="{y:.2f}" r="{r:.2f}" fill="{state_color}" '
            f'fill-opacity="{opacity:g}" stroke="#FFFFFF" stroke-width="2.4"/>'
        )

    def tick(stage: str, px: float, py: float, color: str) -> str:
        x, y = field_position(stage, px, py)
        half = 9.5
        return (
            f'<line class="firing-tick" data-stage="{stage}" x1="{x:.2f}" '
            f'y1="{y - half:.2f}" x2="{x:.2f}" y2="{y + half:.2f}" '
            f'stroke="{color}" stroke-width="5.2" stroke-linecap="round"/>'
        )
    ...
    parts.append(
        '<defs>'
        '<marker id="arrow-neutral" viewBox="0 0 10 10" markerWidth="3.8" markerHeight="3.8" '
        'refX="8.2" refY="5" orient="auto" markerUnits="strokeWidth">'
        f'<path d="M0 0L10 5L0 10Z" fill="{arrow_color}"/></marker>'
        f'<clipPath id="clip-previous"><rect x="{input_a_x:.1f}" y="{input_y:.1f}" width="{input_size:.1f}" height="{input_size:.1f}" rx="10"/></clipPath>'
        f'<clipPath id="clip-current"><rect x="{input_b_x:.1f}" y="{input_y:.1f}" width="{input_size:.1f}" height="{input_size:.1f}" rx="10"/></clipPath>'
        '</defs>'
    )
    parts.append('<rect width="1500" height="430" fill="#FFFFFF"/>')

    # One explicit semantic legend; no repeated under-image labels.
    parts.append(
        f'<g font-family="Arial, \'DejaVu Sans\', sans-serif" font-size="32" fill="{text_color}">'
        f'<circle cx="1110" cy="29" r="9" fill="{state_color}"/>'
        '<text x="1130" y="39" text-anchor="start">STSP state</text>'
        f'<line x1="1320" y1="18" x2="1320" y2="40" stroke="{selected_color}" stroke-width="5.2" stroke-linecap="round"/>'
        '<text x="1340" y="39" text-anchor="start">firing</text>'
        '</g>'
    )

    # The two real external inputs are labelled once above their raster tiles.
    parts.append(
        f'<g font-family="Arial, \'DejaVu Sans\', sans-serif" font-size="32" '
        f'font-weight="700" text-anchor="middle" fill="{text_color}">'
        f'<text x="{stage_centres["inherited"]:.1f}" y="38">Previous A</text>'
        f'<text x="{merge_x:.1f}" y="38">Current B</text>'
        '</g>'
    )
    parts.append(
        f'<image id="input-previous" data-image-id="{role_meta["A"]["image_id"]}" '
        f'x="{input_a_x:.1f}" y="{input_y:.1f}" width="{input_size:.1f}" height="{input_size:.1f}" '
        f'href="data:image/png;base64,{a_b64}" image-rendering="pixelated" '
        'clip-path="url(#clip-previous)"/>'
    )
    parts.append(
        f'<image id="input-current" data-image-id="{role_meta["B"]["image_id"]}" '
        f'x="{input_b_x:.1f}" y="{input_y:.1f}" width="{input_size:.1f}" height="{input_size:.1f}" '
        f'href="data:image/png;base64,{b_b64}" image-rendering="pixelated" '
        'clip-path="url(#clip-current)"/>'
    )
    parts.append(
        f'<rect x="{input_a_x:.1f}" y="{input_y:.1f}" width="{input_size:.1f}" height="{input_size:.1f}" '
        f'rx="10" fill="none" stroke="{border_color}" stroke-width="2.0"/>'
    )
    parts.append(
        f'<rect x="{input_b_x:.1f}" y="{input_y:.1f}" width="{input_size:.1f}" height="{input_size:.1f}" '
        f'rx="10" fill="none" stroke="{border_color}" stroke-width="2.0"/>'
    )

    # Four identical field frames: no coloured card backgrounds and no
    # upstream/downstream vertical offset.
    stages = (
        ("inherited", "Inherited STSP", "upstream", state_color),
        ("selected", "Selected firing", "upstream", selected_color),
        ("downstream", "Downstream firing", "downstream", downstream_color),
        ("successor", "Successor STSP", "downstream", state_color),
    )
    for stage, title, template, edge in stages:
        parts.append(
            f'<rect class="stage-box" data-stage="{stage}" data-template="{template}" '
            f'x="{box_x[stage]:.1f}" y="{box_y:.1f}" width="{box_w:.1f}" height="{box_h:.1f}" '
            f'rx="12" fill="#FFFFFF" stroke="{edge}" stroke-width="2.6"/>'
        )
        parts.append(
            f'<text class="stage-label" data-stage="{stage}" x="{stage_centres[stage]:.1f}" '
            f'y="{title_y:.1f}" text-anchor="middle" font-family="Arial, \'DejaVu Sans\', sans-serif" '
            f'font-size="32" font-weight="700" fill="{edge}">{title}</text>'
        )

    # Input arrows are vertical; the causal chain itself is horizontal.
    parts.append(
        f'<path class="input-arrow" d="M{stage_centres["inherited"]:.1f} {input_y + input_size:.1f} '
        f'V{box_y:.1f}" fill="none" stroke="{arrow_color}" stroke-width="5.0" marker-end="url(#arrow-neutral)"/>'
    )
    parts.append(
        f'<path class="input-arrow" d="M{merge_x:.1f} {input_y + input_size:.1f} V{merge_y - 17:.1f}" '
        f'fill="none" stroke="{arrow_color}" stroke-width="5.0" marker-end="url(#arrow-neutral)"/>'
    )
    parts.append(
        f'<path class="causal-arrow" d="M{box_x["inherited"] + box_w:.1f} {merge_y:.1f} '
        f'H{merge_x - 17:.1f}" fill="none" stroke="{arrow_color}" stroke-width="5.0" marker-end="url(#arrow-neutral)"/>'
    )
    parts.append(
        f'<circle class="merge-node" cx="{merge_x:.1f}" cy="{merge_y:.1f}" r="16" '
        f'fill="#FFFFFF" stroke="{arrow_color}" stroke-width="3.0"/>'
    )
    parts.append(
        f'<text x="{merge_x:.1f}" y="{merge_y + 11:.1f}" text-anchor="middle" '
        f'font-family="Arial, \'DejaVu Sans\', sans-serif" font-size="30" font-weight="700" fill="{arrow_color}">+</text>'
    )
    parts.append(
        f'<path class="causal-arrow" d="M{merge_x + 17:.1f} {merge_y:.1f} '
        f'H{box_x["selected"]:.1f}" fill="none" stroke="{arrow_color}" stroke-width="5.0" marker-end="url(#arrow-neutral)"/>'
    )
    parts.append(
        f'<path class="causal-arrow" d="M{box_x["selected"] + box_w:.1f} {merge_y:.1f} '
        f'H{box_x["downstream"]:.1f}" fill="none" stroke="{arrow_color}" stroke-width="5.0" marker-end="url(#arrow-neutral)"/>'
    )
    parts.append(
        f'<path class="causal-arrow" d="M{box_x["downstream"] + box_w:.1f} {merge_y:.1f} '
        f'H{box_x["successor"]:.1f}" fill="none" stroke="{arrow_color}" stroke-width="5.0" marker-end="url(#arrow-neutral)"/>'
    )

    # Each field contains one glyph grammar only.
    for node in UPSTREAM_NODES:
        parts.append(dot("inherited", node, 1.0))
    for px, py in SELECTED_NODES:
        parts.append(tick("selected", px, py, selected_color))
    for node in DOWNSTREAM_NODES:
        color = downstream_color if (node[0], node[1]) in DOWNSTREAM_CYAN else selected_color
        parts.append(tick("downstream", node[0], node[1], color))
    for node_index, node in enumerate(DOWNSTREAM_NODES):
        opacity = 0.72 if node_index in (4, 7, 10) else 1.0
        parts.append(dot("successor", node, opacity))
...
def _export_pdf_and_png(
    ...
    with tempfile.TemporaryDirectory(
        ...
        writer.metadata = {
            "/Title": "manuscript_fig4_reader_first_v3",
...
def _render_figure(
    ...
    with plt.rc_context(
        {
            **VECTOR_TEXT_RCPARAMS,
            "svg.hashsalt": "net_torch_manuscript_fig4_reader_first_v3",
...
def _render_qa(
    ...
    checks = {
        ...
        "svg_raster_inputs": svg_image_count == 2,
    ...
    return {
        ...
        "raster_note": "the two raster image elements are the real Fig.3-identical MNIST inputs A and B embedded from the persisted stimulus CSV",
        "pdf": {
            "path": str(pdf),
...
def _candidate_tables(payload: Mapping[str, Any]) -> dict[str, pd.DataFrame]:
    ...
    return {
        ...
        "panel_g_stimuli": payload["stimuli"].copy(),
    }
...
def _source_mapping(
    ...
    for panel_id, endpoint_label, data_key, stats_key, manifest_key, mapping_text in (
        ...
        (
            ...
            "Frozen descriptive early-event endpoint; bars show frozen means and 95% CIs; exact window remains in source statistics.",
        ),
        (
            "d",
            "Event reduction",
            ...
            "Frozen early-event contrast endpoints; bars show frozen means and 95% CIs; exact window remains in source statistics.",
        ),
        (
            "e",
    ...
    stimulus_relative = str(spec["panels"]["g"]["source"])
    g_path = _repo_root() / g_relative
    stimulus_root = _repo_root() / spec["stimulus_bundle"]
    rows.append(
        {
            "candidate_figure": DISPLAY_NAME,
            "candidate_panel": "g",
            "reader_label": "Previous input A and current input B rasters",
            "technical_endpoint_or_object": "28x28 normalized_intensity matrices; A image_id 9301 label 1; B image_id 3536 label 0",
            "parent_source_scope": "frozen stimulus bundle (manuscript Fig.3 parent)",
            "parent_data_path": stimulus_relative,
            "parent_data_sha256": _sha256(stimulus_root / stimulus_relative),
            "parent_statistics_path": "",
            "parent_statistics_sha256": "",
            "parent_upstream_manifest": "meta/panel_a_source_manifest.csv",
            "parent_upstream_manifest_sha256": "listed per source file",
            "independent_unit": "not_applicable (single persisted stimulus set)",
            "included_networks": "not_applicable",
            "mapping": "persisted pixel matrix -> black-background grayscale PNG -> embedded raster; identical to the Same B presented in Fig.3; image hashes 4086585a... (A) and 8a707dee... (B)",
        }
    )
    rows.append(
        {
            ...
            "parent_source_scope": "generated candidate asset (deterministic from the stimulus CSV)",
            "parent_data_path": g_relative,
            "parent_data_sha256": _sha256(g_path),
            "parent_statistics_path": "",
            "parent_statistics_sha256": "",
            "parent_upstream_manifest": "meta/panel_g_asset_manifest.csv",
            "parent_upstream_manifest_sha256": "listed in the asset manifest",
            "independent_unit": "not_applicable",
            "included_networks": "not_applicable",
            "mapping": "single left-to-right causal chain; Previous input A and Current input B are real persisted rasters; inherited/successor STSP fields contain teal state dots only; selected/downstream firing fields contain vertical spike ticks only; one merge junction combines current input with inherited state",
        }
    )
    return pd.DataFrame(rows)
...
def build_candidate(
    *,
    ...
    expected_stimulus = (repo_root / spec["stimulus_bundle"]).resolve()
    ...
    stimulus_before = _snapshot_selected(
        expected_stimulus, (stimulus_relative,), source_scope="stimulus_bundle"
    )
    g_relative = str(spec["g_asset"])
    g_asset_path = repo_root / g_relative
    parent_before = pd.concat(
        [bundle_before, overlap_before, swap_before, stimulus_before],
        ignore_index=True,
    )
    ...
    reader = BundleReader(
        roots={
            ...
            "stimulus_bundle": expected_stimulus,
        },
        expected_roots={
        ...
        allowed_files={
            ...
            "stimulus_bundle": {stimulus_relative},
        },
    )
    ...
    g_asset_bytes = _build_g_asset_svg(payload, spec)
    if g_asset_path.is_file():
        if g_asset_path.read_bytes() != g_asset_bytes:
            raise RuntimeError(
                "generated Fig.4g asset differs from the persisted candidate asset"
            )
    else:
        g_asset_path.write_bytes(g_asset_bytes)
    (output_dir / "figures" / "qa" / "manuscript_fig4g_asset.svg").write_bytes(
        g_asset_bytes
    )
    ...
    stimulus_manifest_rows: list[dict[str, Any]] = []
    for role in ("A", "B"):
        meta = payload["stimulus_meta"][role]
        stimulus_manifest_rows.append(
            {
                ...
                "render_occurrences": "|".join(str(value) for value in meta["render_occurrences"]),
                "n_pixels": meta["n_pixels"],
                "source_path": stimulus_relative,
                "source_sha256": _sha256(expected_stimulus / stimulus_relative),
                "rendering": "28x28 normalized_intensity -> black-background grayscale PNG, nearest-neighbour upscale",
                "consistency": "identical to the Same B presented in manuscript Fig.3",
            }
        )
    pd.DataFrame(stimulus_manifest_rows).to_csv(
        output_dir / "meta" / "panel_g_stimulus_manifest.csv", index=False
    )
    pd.DataFrame(
        [
            {
                ...
                "asset_bytes": g_asset_path.stat().st_size,
                "viewBox": spec["g_asset_viewbox"],
                "asset_role": "Fig.4-end conceptual synthesis of history-dependent successor formation",
                "semantics": "single left-to-right causal chain; real MNIST rasters for previous (A) and current (B) inputs; one merge junction; STSP fields use teal circles only and firing fields use vertical ticks only; upstream and downstream pairs share aligned templates",
    ...
    if not check_only:
        ...
        outputs = _render_figure(spec, payload, output_dir / "figures", g_asset_bytes)
    ...
    stimulus_after = _snapshot_selected(
        expected_stimulus, (stimulus_relative,), source_scope="stimulus_bundle"
    )
    parent_after = pd.concat(
        [bundle_after, overlap_after, swap_after, stimulus_after],
        ignore_index=True,
    )
    ...
    parent_integrity = {
        ...
        "status": "passed" if parent_unchanged else "failed",
            ...
            "stimulus_bundle": str(expected_stimulus),
        },
    ...
    summary = {
        ...
        "panel_g_audit": {
            "previous_input": payload["stimulus_meta"]["A"],
            "current_input": payload["stimulus_meta"]["B"],
            "asset_sha256": _sha256(g_asset_path),
            "topology": "single left-to-right chain with two vertical external-input arrows and one current-input/state merge junction",
            "stage_grammar": "inherited STSP (teal circles) -> selected firing (navy ticks) -> downstream firing (navy/cyan ticks) -> successor STSP (teal circles)",
            "alignment": "all four stage fields share the same baseline and outer geometry; upstream and downstream pairs share their internal templates",
        },
        "outputs": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/paper_figure_candidates/manuscript_fig4_reader_first_v3",
    )
