...
CANDIDATE_VERSION = "manuscript_fig4_reader_first_v2"
DISPLAY_NAME = "Fig.4"
...
SPEC_PATH = (
    ...
    / "specs"
    / "manuscript_fig4_reader_first_v2.json"
)
...
def _build_g_asset_svg(payload: Mapping[str, Any], spec: Mapping[str, Any]) -> bytes:
    ...
    a_b64 = _raster_png_base64(images["A"])
    b_b64 = _raster_png_base64(images["B"])

    cards = [
        {
            "cx": 215,
            "label": "Previous input",
            "title": "Inherited STSP",
            "title_color": "#009E73",
            "fill": "#E8F5F1",
            "edge": "#009E73",
            "arrow_color": "#009E73",
            "kind": "raster",
            "b64": a_b64,
            "image_id": role_meta["A"]["image_id"],
        },
        {
            "cx": 585,
            "label": "Current input",
            "title": "Selected firing",
            "title_color": "#0072B2",
            "fill": "#EAF4FA",
            "edge": "#0072B2",
            "arrow_color": "#0072B2",
            "kind": "raster",
            "b64": b_b64,
            "image_id": role_meta["B"]["image_id"],
        },
        {
            "cx": 955,
            "label": "Selected firing",
            "title": "Downstream firing",
            "title_color": "#0072B2",
            "fill": "#EAF4FA",
            "edge": "#56B4E9",
            "arrow_color": "#0072B2",
        ...
        {
            "cx": 1325,
            "label": "Downstream firing",
            "title": "Successor STSP",
            "title_color": "#CC79A7",
            "fill": "#F7EAF2",
            "edge": "#CC79A7",
            "arrow_color": "#CC79A7",
    ...
    box_y_upstream = 196.0
    box_y_downstream = 212.0
    box_w = 206.0
    box_h = 166.0
    field_scale = box_h / 180.0
    field_origin = (20.0, 0.0)
    tile_size = 104.0
    tile_y = 52.0
    tile_field = 72.0
    tile_scale = tile_field / 180.0

    def dot(cx: float, by: float, node: tuple[float, float, float], color: str, opacity: float) -> str:
        px, py, radius = node
        x = cx - box_w / 2.0 + field_origin[0] + px * field_scale
        y = by + field_origin[1] + py * field_scale
        r = radius * field_scale
        fill = f'fill="{color}" fill-opacity="{opacity:g}" '
        return (
            f'<circle cx="{x:.2f}" cy="{y:.2f}" r="{r:.2f}" '
            f'{fill}stroke="#FFFFFF" stroke-width="2.0"/>'
        )

    def tick(cx: float, by: float, px: float, py: float, color: str, half: float = 8.0, width: float = 3.4) -> str:
        x = cx - box_w / 2.0 + field_origin[0] + px * field_scale
        y = by + field_origin[1] + py * field_scale
        return (
            f'<line x1="{x:.2f}" y1="{y - half * field_scale:.2f}" '
            f'x2="{x:.2f}" y2="{y + half * field_scale:.2f}" '
            f'stroke="{color}" stroke-width="{width:.1f}" stroke-linecap="round"/>'
        )

    def tile_tick(px: float, py: float, color: str) -> str:
        x = tile_field / 2.0 + px * tile_scale
        y = tile_field / 2.0 + py * tile_scale
        return (
            f'<line x1="{x:.2f}" y1="{y - 3.2:.2f}" '
            f'x2="{x:.2f}" y2="{y + 3.2:.2f}" '
    ...
    parts.append(
        ...
        'width="1560" height="420" viewBox="0 0 1560 420">'
    )
    parts.append(
        "  <defs>"
        '    <marker id="arrow-teal" viewBox="0 0 10 10" markerWidth="3.2" markerHeight="3.2" refX="8.5" refY="5" orient="auto" markerUnits="strokeWidth"><path d="M0 0L10 5L0 10Z" fill="#009E73"/></marker>'
        '    <marker id="arrow-navy" viewBox="0 0 10 10" markerWidth="3.2" markerHeight="3.2" refX="8.5" refY="5" orient="auto" markerUnits="strokeWidth"><path d="M0 0L10 5L0 10Z" fill="#0072B2"/></marker>'
        '    <marker id="arrow-cyan" viewBox="0 0 10 10" markerWidth="3.2" markerHeight="3.2" refX="8.5" refY="5" orient="auto" markerUnits="strokeWidth"><path d="M0 0L10 5L0 10Z" fill="#56B4E9"/></marker>'
        '    <marker id="arrow-slate" viewBox="0 0 10 10" markerWidth="3.2" markerHeight="3.2" refX="8.5" refY="5" orient="auto" markerUnits="strokeWidth"><path d="M0 0L10 5L0 10Z" fill="#CC79A7"/></marker>'
        '    <clipPath id="clip-previous"><rect x="163" y="52" width="104" height="104" rx="12"/></clipPath>'
        '    <clipPath id="clip-current"><rect x="533" y="52" width="104" height="104" rx="12"/></clipPath>'
        "  </defs>"
    )
    parts.append('<rect width="1560" height="420" fill="#FFFFFF"/>')
    parts.append(
        '<g font-family="Arial, \'DejaVu Sans\', sans-serif" text-anchor="middle" font-size="17" font-weight="700">'
    )
    for card in cards:
        parts.append(
            f'<text x="{card["cx"]}" y="27" fill="{card["title_color"]}">{card["title"]}</text>'
        )
    parts.append("</g>")
    for card in cards:
        parts.append(
            f'<rect x="{card["cx"] - 175}" y="40" width="350" height="350" rx="16" '
            f'fill="{card["fill"]}" stroke="{card["edge"]}" stroke-width="2.2"/>'
        )
    # top driver objects: real rasters for cards 1-2, handoff tiles for 3-4
    for index, card in enumerate(cards, start=1):
        x = card["cx"] - tile_size / 2.0
        if card["kind"] == "raster":
            clip = "clip-previous" if index == 1 else "clip-current"
            parts.append(
                f'<image id="input-{"previous" if index == 1 else "current"}" '
                f'x="{x:.0f}" y="{tile_y:.0f}" width="{tile_size:.0f}" height="{tile_size:.0f}" '
                f'href="data:image/png;base64,{card["b64"]}" image-rendering="pixelated" '
                f'clip-path="url(#{clip})"/>'
            )
        else:
            parts.append(
                f'<rect class="handoff-tile" x="{x:.0f}" y="{tile_y:.0f}" '
                f'width="{tile_size:.0f}" height="{tile_size:.0f}" rx="12" fill="#FFFFFF" '
                f'stroke="#D9D9D9" stroke-width="1.6"/>'
            )
            nodes = SELECTED_NODES if index == 3 else [(n[0], n[1]) for n in DOWNSTREAM_NODES]
            for px, py in nodes:
                if index == 4:
                    color = "#56B4E9" if (px, py) in DOWNSTREAM_CYAN else "#0072B2"
                else:
                    color = "#0072B2"
                parts.append(
                    f'<g transform="translate({x:.0f},{tile_y:.0f})">{tile_tick(px, py, color)}</g>'
                )
    # driver labels
    parts.append(
        '<g font-family="Arial, \'DejaVu Sans\', sans-serif" text-anchor="middle" font-size="13" fill="#666666">'
    )
    for card in cards:
        parts.append(f'<text x="{card["cx"]}" y="164">{card["label"]}</text>')
    parts.append("</g>")
    # intra-card arrows: top driver -> bottom result
    marker_by_color = {
        "#009E73": "teal",
        "#0072B2": "navy",
        "#56B4E9": "cyan",
    ...
    for card in cards:
        parts.append(
            f'<path d="M{card["cx"]} 172V190" fill="none" stroke="{card["arrow_color"]}" '
            f'stroke-width="3.6" marker-end="url(#arrow-{marker_by_color[card["arrow_color"]]})"/>'
        )
    # result boxes
    for index, card in enumerate(cards, start=1):
        by = box_y_upstream if index <= 2 else box_y_downstream
        parts.append(
            f'<rect class="result-box" data-card="{index}" x="{card["cx"] - box_w / 2.0:.0f}" '
            f'y="{by:.0f}" width="{box_w:.0f}" height="{box_h:.0f}" rx="12" fill="#FFFFFF" '
            f'stroke="{card["edge"]}" stroke-width="2.4"/>'
        )
    # result content
    for index, card in enumerate(cards, start=1):
        by = box_y_upstream if index <= 2 else box_y_downstream
        if index == 1:
            for node in UPSTREAM_NODES:
                parts.append(dot(card["cx"], by, node, "#009E73", 1.0))
        elif index == 2:
            for node in UPSTREAM_NODES:
                faded = (node[0], node[1], node[2] * 0.62)
                parts.append(dot(card["cx"], by, faded, "#009E73", 0.30))
            for px, py in SELECTED_NODES:
                parts.append(tick(card["cx"], by, px, py, "#0072B2"))
        elif index == 3:
            for node in DOWNSTREAM_NODES:
                color = "#56B4E9" if (node[0], node[1]) in DOWNSTREAM_CYAN else "#0072B2"
                parts.append(tick(card["cx"], by, node[0], node[1], color))
        else:
            for node_index, node in enumerate(DOWNSTREAM_NODES):
                opacity = 0.65 if node_index in (4, 7, 10) else 1.0
                parts.append(dot(card["cx"], by, node, "#CC79A7", opacity))
    # inter-card handoff arrows
    parts.append(
        '<path d="M318 279H482" fill="none" stroke="#009E73" stroke-width="4" marker-end="url(#arrow-teal)"/>'
    )
    parts.append(
        '<path d="M688 279L903 110" fill="none" stroke="#0072B2" stroke-width="4" marker-end="url(#arrow-navy)"/>'
    )
    parts.append(
        '<path d="M1098 295L1273 110" fill="none" stroke="#56B4E9" stroke-width="4" marker-end="url(#arrow-cyan)"/>'
    )
    parts.append("</svg>")
    ...
    rows.append(
        {
            ...
            "included_networks": "not_applicable",
            "mapping": "four cards, each pairing a top driver (Previous input, Current input, Selected firing handoff, Downstream firing handoff) with the bottom result (Inherited STSP, Selected firing, Downstream firing, Successor STSP); state dots in cards 1 and 4, spike ticks in cards 2 and 3; cards 3-4 share the downstream template",
        }
    )
    return pd.DataFrame(rows)
    ...
    *,
        ...
        [
            {
                ...
                "semantics": "four cards each pair a top driver with the state or activity it produces; real MNIST rasters for previous (A) and current (B) inputs; state dots vs firing ticks; upstream template in cards 1-2 and one shared downstream template in cards 3-4",
    ...
    summary = {
        ...
        "panel_g_audit": {
            ...
            "asset_sha256": _sha256(g_asset_path),
            "card_grammar": "top driver -> bottom result in all four cards",
            "glyph_grammar": "state dots in cards 1 and 4; spike ticks in cards 2 and 3 and the handoff tiles",
            "downstream_template": "cards 3 and 4 share the shifted result-box position and the downstream node template",
        },
        "outputs": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/paper_figure_candidates/manuscript_fig4_reader_first_v2",
    )
