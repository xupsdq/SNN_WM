...
CANDIDATE_VERSION = "manuscript_fig3_reader_first_v2"
...
SPEC_PATH = (
    ...
    / "specs"
    / "manuscript_fig3_reader_first_v2.json"
)
...
def _plot_panel_a(
    ...
    for role in ("A", "C"):
        ...
        choice_xy = tuple(float(value) for value in choice_center[role])
        _draw_stimulus(
            axis,
            images[role],
            history_box,
        ...
        bracket_x = float(layout["equality"]["bracket_x"])
        _draw_arrow(
            axis,
            (bracket_x + 1.0, y),
            (post_box[0] - 0.8, y),
    ...
    axis.text(
        float(inherited_bbox["A"][0]) + float(inherited_bbox["A"][2]) / 2.0,
        header_y,
        labels["inherited_state"],
        ha="center",
        va="top",
    ...
    axis.text(
        float(b_bbox["A"][0]) + float(b_bbox["A"][2]) / 2.0,
        header_y,
        labels["same_b"],
        ha="center",
        va="top",
    ...
    axis.text(
        float(post_bbox["A"][0]) + float(post_bbox["A"][2]) / 2.0,
        header_y,
        labels["state_after_b"],
        ha="center",
        va="top",
    ...
    bracket_x = float(equality["bracket_x"])
    top_y, bottom_y = lane_y["A"], lane_y["C"]
    current_right = float(b_bbox["A"][0]) + float(b_bbox["A"][2])
    ...
    equals_x = float(equality["equals_center_x"])
...
def _plot_joint_density(
    ...
    threshold_95 = float(contours.loc[0.95, "density_threshold"])
    threshold_80 = float(contours.loc[0.8, "density_threshold"])
    threshold_50 = float(contours.loc[0.5, "density_threshold"])
    levels = [threshold_95, threshold_80, threshold_50, float(aggregate.max()) * 1.001]
    if not all(first < second for first, second in zip(levels, levels[1:])):
        raise ValueError(f"Fig.3c contour levels are not strictly ordered: {levels}")
    ...
    axis.contourf(
        ...
        levels=levels,
        colors=[
            matplotlib.colors.to_rgba(density_color, 0.22),
            matplotlib.colors.to_rgba(density_color, 0.42),
            matplotlib.colors.to_rgba(density_color, 0.68),
        ],
        antialiased=True,
        zorder=1,
    )
...
def _render_figure(
    ...
    with plt.rc_context(
        {
            **VECTOR_TEXT_RCPARAMS,
            "svg.hashsalt": "net_torch_manuscript_fig3_reader_first_v2",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/paper_figure_candidates/manuscript_fig3_reader_first_v2",
    )
