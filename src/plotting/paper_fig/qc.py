...
def _check_fig1_specifics(
    ...
    if figure_id != "fig1":
        return
    canvas = spec.get("canvas_mm") or {}
    if float(canvas.get("width", 0)) == 165 and float(canvas.get("height", 0)) == 152:
        passes.append("Fig.1 canvas is 165 x 152 mm")
    else:
        failures.append(f"Fig.1 canvas must be 165 x 152 mm, found {canvas}")
...
def _check_fig1_geometry(spec: Mapping[str, Any], panels: Mapping[str, Any], passes: list[str], warnings: list[str], failures: list[str]) -> None:
    ...
    if len(pos) != 5:
        return
    expected = {
        "A": {"x": 2.0, "y": 2.0, "w": 161.0, "h": 48.0},
        "B": {"x": 2.0, "y": 52.0, "w": 79.5, "h": 48.0},
        "C": {"x": 83.5, "y": 52.0, "w": 79.5, "h": 48.0},
        "D": {"x": 2.0, "y": 102.0, "w": 79.5, "h": 48.0},
        "E": {"x": 83.5, "y": 102.0, "w": 79.5, "h": 48.0},
    }
    for panel_id, target in expected.items():
        if _fig_target_box_near(pos[panel_id], target):
            passes.append(f"Fig.1{panel_id}: target full-panel bbox matches spec")
        else:
            failures.append(f"Fig.1{panel_id}: target full-panel bbox must be {target}, found {pos[panel_id]}")
    def _edge(pid: str, key: str) -> float:
        return float(pos[pid].get(key, 0.0))
    if _near(_edge("A", "x"), 2.0, tol=0.05) and _near(_edge("A", "w"), 161.0, tol=0.05):
        passes.append("Fig.1A spans the full two-column width")
    else:
        failures.append(f"Fig.1A must span x=2, w=161 mm, found {pos['A']}")
    ...
    bottom_margin = min(float((spec.get("canvas_mm") or {}).get("height", 0)) - (_edge(pid, "y") + _edge(pid, "h")) for pid in ("D", "E"))
    if bottom_margin >= 2.0:
        passes.append(f"Fig.1 bottom full-panel margin is {bottom_margin:.1f} mm")
    else:
        failures.append(f"Fig.1 bottom full-panel margin too small ({bottom_margin:.1f} mm)")
...
def _check_fig2_specifics(
    ...
    canvas = spec.get("canvas_mm") or {}
    if float(canvas.get("width", 0)) == 165 and float(canvas.get("height", 0)) == 152:
        passes.append("Fig.2 canvas is 165 x 152 mm")
    else:
        failures.append(f"Fig.2 canvas must be 165 x 152 mm, found {canvas}")
...
def _check_fig3_standalone_contract(
    ...
    figure_id = "fig3"
    contract = str(spec.get("scientific_contract", ""))
    if contract == "sequence_chunk_rescue_v1":
        _check_fig3_sequence_chunk_rescue_contract(spec, panels, output_dir, render_metadata, passes, warnings, failures)
        return
    if contract == "morphology_function_boundary_v1":
        _check_fig3_morphology_function_boundary_contract(spec, panels, output_dir, render_metadata, passes, warnings, failures)
        return
    canvas = spec.get("canvas_mm") or {}
...
def _check_fig3_morphology_function_boundary_contract(
    spec: Mapping[str, Any],
    panels: Mapping[str, Any],
    output_dir: Path,
    render_metadata: Mapping[str, Mapping[str, Any]],
    ...
    if float(canvas.get("width", 0)) == 165 and float(canvas.get("height", 0)) == 126:
        passes.append("Fig.3 canvas is 165 x 126 mm")
    else:
        failures.append(f"Fig.3 canvas must be 165 x 126 mm, found {canvas}")
    expected_order = ["A", "B", "C", "D"]
    if list(spec.get("reading_order") or []) == expected_order and set(panels.keys()) == set(expected_order):
        passes.append("Fig.3 uses A-D reading order for morphology/function boundary")
    else:
        failures.append(f"Fig.3 must contain panels/reading_order A-D, found panels={sorted(panels.keys())}, reading_order={spec.get('reading_order')}")
    expected_renderers = {
        "A": "render_fig3_morphology_serial_profile",
        "B": "render_fig3_boundary_heatmap",
        "C": "render_fig3_boundary_heatmap",
        "D": "render_fig3_morphology_function_coupling",
    }
    for panel_id, renderer in expected_renderers.items():
        found = (panels.get(panel_id) or {}).get("renderer")
        if found == renderer:
            passes.append(f"Fig.3{panel_id}: renderer={renderer}")
        else:
            failures.append(f"Fig.3{panel_id}: expected renderer {renderer}, found {found}")
    expected_slots = {
        "A": {"x": 12, "y": 6, "w": 147, "h": 88},
        "B": {"x": 12, "y": 102, "w": 45, "h": 50},
        "C": {"x": 63, "y": 102, "w": 45, "h": 50},
        "D": {"x": 114, "y": 102, "w": 45, "h": 50},
    }
    for panel_id, expected in expected_slots.items():
        ...
        if _fig3_mm_box_close(pos, expected):
            passes.append(f"Fig.3{panel_id}: slot mm coordinates match boundary specification")
        else:
            failures.append(f"Fig.3{panel_id}: slot coordinates differ from boundary specification, found {pos}")

    panel_data: dict[str, pd.DataFrame] = {}
    for panel_id in expected_order:
        paths = panel_output_paths(output_dir, figure_id, panel_id)
        missing = [name for name in ("panel_data", "stats", "sources") if not paths[name].exists()]
        if missing:
            failures.append(f"Fig.3{panel_id}: missing adapter outputs {missing}")
        ...
        if run_mode:
            ...
            failures.append(f"Fig.3{panel_id}: run_mode missing")
        if n_networks == 1 or run_mode == "single_network_draft":
            warnings.append(f"Fig.3{panel_id}: single_network_draft n_networks=1; smoke/draft only, not final manuscript statistics")
        elif n_networks > 1:
            passes.append(f"Fig.3{panel_id}: n_networks={n_networks}")
        else:
            warnings.append(f"Fig.3{panel_id}: n_networks not recorded")
        source_text = _panel_sources(output_dir, figure_id, panel_id)
        if "data/intermediates" in source_text or "data\\intermediates" in source_text:
            failures.append(f"Fig.3{panel_id}: plotting source manifest must not read data/intermediates")
        else:
            passes.append(f"Fig.3{panel_id}: plot-only sources avoid data/intermediates")

    a_df = panel_data.get("A")
    if a_df is not None:
        metrics = set(a_df.get("metric", pd.Series(dtype=str)).astype(str))
        layers = set(a_df.get("layer", pd.Series(dtype=str)).astype(str))
        serial = pd.to_numeric(a_df.get("serial_position", pd.Series(dtype=float)), errors="coerce").dropna()
        if "morphology_support_mass" in metrics:
            passes.append("Fig.3A uses item-wise morphology_support_mass")
        else:
            failures.append(f"Fig.3A missing morphology_support_mass, found {sorted(metrics)}")
        if "layer1" in layers:
            passes.append("Fig.3A reads L1 morphology")
        else:
            failures.append(f"Fig.3A must include layer1 rows, found {sorted(layers)}")
        if len(set(serial.astype(int))) >= 2:
            passes.append("Fig.3A has multiple serial item positions")
        else:
            warnings.append(f"Fig.3A has limited serial positions in smoke data: {sorted(set(serial.astype(int))) if not serial.empty else []}")

    b_df = panel_data.get("B")
    if b_df is not None:
        metrics = set(b_df.get("metric", pd.Series(dtype=str)).astype(str))
        if "multi_item_retention_index" in metrics:
            passes.append("Fig.3B uses multi_item_retention_index")
        else:
            failures.append(f"Fig.3B missing multi_item_retention_index, found {sorted(metrics)}")
        if {"seq_len", "delay_ms"}.issubset(b_df.columns):
            passes.append("Fig.3B records K x delay coordinates")
        else:
            failures.append("Fig.3B must record seq_len and delay_ms")

    c_df = panel_data.get("C")
    if c_df is not None:
        metrics = set(c_df.get("metric", pd.Series(dtype=str)).astype(str))
        if "accessible_item_count" in metrics:
            passes.append("Fig.3C uses accessible_item_count")
        else:
            failures.append(f"Fig.3C missing accessible_item_count, found {sorted(metrics)}")
        if {"seq_len", "delay_ms"}.issubset(c_df.columns):
            passes.append("Fig.3C records K x delay coordinates")
        else:
            failures.append("Fig.3C must record seq_len and delay_ms")

    d_df = panel_data.get("D")
    if d_df is not None:
        metrics = set(d_df.get("metric", pd.Series(dtype=str)).astype(str))
        required_cols = {"morphology_support_p", "functional_gain_norm", "serial_position"}
        if "G_i_norm" in metrics:
            passes.append("Fig.3D uses normalized item-level gain G_i_norm")
        else:
            failures.append(f"Fig.3D missing G_i_norm, found {sorted(metrics)}")
        if required_cols.issubset(d_df.columns):
            passes.append("Fig.3D contains item-level morphology/function coupling columns")
        else:
            failures.append(f"Fig.3D missing coupling columns {sorted(required_cols - set(d_df.columns))}")

    if "E" in panels or "F" in panels:
        failures.append("Fig.3 main figure must not contain E/F panels")
    else:
        passes.append("Fig.3 E/F panels are absent from the main figure spec")
    if render_metadata:
        clipped = {
...
def _check_fig3_sequence_chunk_rescue_contract(
    spec: Mapping[str, Any],
    panels: Mapping[str, Any],
    output_dir: Path,
    render_metadata: Mapping[str, Mapping[str, Any]],
    ...
    figure_id = "fig3"
    canvas = spec.get("canvas_mm") or {}
    expected_canvas = (spec.get("qc_requirements") or {}).get("canvas_mm") or {"width": 165, "height": 118}
    expected_width = float(expected_canvas.get("width", 165))
    expected_height = float(expected_canvas.get("height", 118))
    if float(canvas.get("width", 0)) == expected_width and float(canvas.get("height", 0)) == expected_height:
        passes.append(f"Fig.3 canvas is {expected_width:.0f} x {expected_height:.0f} mm")
    else:
        failures.append(f"Fig.3 canvas must be {expected_width:.0f} x {expected_height:.0f} mm, found {canvas}")
    expected_order = ["A", "B", "C", "D", "E"]
    if list(spec.get("reading_order") or []) == expected_order and set(panels.keys()) == set(expected_order):
        passes.append("Fig.3 uses A-E reading order for two-row sequence chunk rescue layout")
    else:
        failures.append(f"Fig.3 must contain panels/reading_order A-E, found panels={sorted(panels.keys())}, reading_order={spec.get('reading_order')}")
    expected_renderers = {
        "A": "render_fig3_morphology_capacity",
        "B": "render_fig3_access_serial_profile",
        "C": "render_fig3_cue_specificity_target_profile",
        "D": "render_fig3_rescue_fraction",
        "E": "render_fig3_boundary_heatmap",
    }
    for panel_id, renderer in expected_renderers.items():
        found = (panels.get(panel_id) or {}).get("renderer")
        if found == renderer:
            ...
            failures.append(f"Fig.3{panel_id}: expected renderer {renderer}, found {found}")
    expected_slots = {
        "A": {"x": 2.0, "y": 10.0, "w": 79.5, "h": 48.0},
        "B": {"x": 83.5, "y": 10.0, "w": 79.5, "h": 48.0},
        "C": {"x": 2.0, "y": 60.0, "w": 52.333, "h": 48.0},
        "D": {"x": 56.333, "y": 60.0, "w": 52.333, "h": 48.0},
        "E": {"x": 110.667, "y": 60.0, "w": 52.333, "h": 48.0},
    }
    for panel_id, expected in expected_slots.items():
        pos = (panels.get(panel_id) or {}).get("position_mm") or {}
        if _fig3_mm_box_close(pos, expected):
            passes.append(f"Fig.3{panel_id}: slot mm coordinates match rescue specification")
        else:
            failures.append(f"Fig.3{panel_id}: slot coordinates differ from rescue specification, found {pos}")
    ...
    if a_df is not None:
        metrics = set(a_df.get("metric", pd.Series(dtype=str)).astype(str))
        if "N_eff" in metrics:
            passes.append("Fig.3A uses L1 morphology capacity N_eff")
        else:
            failures.append(f"Fig.3A missing N_eff, found {sorted(metrics)}")
        if {"seq_len", "delay_ms"}.issubset(a_df.columns):
            passes.append("Fig.3A records K and delay")
        else:
            failures.append("Fig.3A must record seq_len and delay_ms")

    b_df = panel_data.get("B")
    if b_df is not None:
        metrics = set(b_df.get("metric", pd.Series(dtype=str)).astype(str))
        memory_conditions = set(b_df.get("memory_condition", pd.Series(dtype=str)).astype(str))
        serial = pd.to_numeric(b_df.get("serial_position", pd.Series(dtype=float)), errors="coerce").dropna()
        if "target_probability" in metrics:
            passes.append("Fig.3B uses target_probability for weak-cue serial access")
        else:
            failures.append(f"Fig.3B missing target_probability, found {sorted(metrics)}")
        required_memory = {"cue_only", "single_item_memory", "sequence_state"}
        if required_memory.issubset(memory_conditions):
            passes.append("Fig.3B compares cue-only, singleton, and sequence-state access")
        else:
            failures.append(f"Fig.3B missing memory conditions {sorted(required_memory - memory_conditions)}")
        if len(set(serial.astype(int))) >= 5:
            passes.append("Fig.3B covers multiple serial positions")
        else:
            warnings.append(f"Fig.3B has limited serial positions: {sorted(set(serial.astype(int))) if not serial.empty else []}")

    c_df = panel_data.get("C")
    if c_df is not None:
        metrics = set(c_df.get("metric", pd.Series(dtype=str)).astype(str))
        cue_types = set(c_df.get("cue_type", pd.Series(dtype=str)).astype(str))
        states = set(c_df.get("state_condition", pd.Series(dtype=str)).astype(str))
        serial = pd.to_numeric(c_df.get("serial_position", pd.Series(dtype=float)), errors="coerce").dropna()
        if "target_probability" in metrics:
            passes.append("Fig.3C uses target_probability for cue-specific target access")
        else:
            failures.append(f"Fig.3C missing target_probability, found {sorted(metrics)}")
        required_cues = {"matched", "mismatched", "unseen"}
        if required_cues.issubset(cue_types):
            passes.append("Fig.3C compares matched, mismatched, and unseen cues")
        else:
            failures.append(f"Fig.3C missing cue types {sorted(required_cues - cue_types)}")
        if "S_final" in states:
            passes.append("Fig.3C plots S_final cue specificity")
        else:
            failures.append(f"Fig.3C must include S_final rows, found {sorted(states)}")
        observed_k = {int(v) for v in pd.to_numeric(c_df.get("seq_len", pd.Series(dtype=float)), errors="coerce").dropna().unique()}
        observed_delay = {int(v) for v in pd.to_numeric(c_df.get("delay_ms", pd.Series(dtype=float)), errors="coerce").dropna().unique()}
        if observed_k == {7}:
            passes.append("Fig.3C is the K7 cue-specificity control")
        else:
            failures.append(f"Fig.3C must use K7 only, found K={sorted(observed_k)}")
        if observed_delay == {400}:
            passes.append("Fig.3C is the D400 cue-specificity control")
        else:
            failures.append(f"Fig.3C must use delay 400 ms only, found delays={sorted(observed_delay)}")
        if len(set(serial.astype(int))) >= 5:
            passes.append("Fig.3C covers multiple serial positions")
        else:
            warnings.append(f"Fig.3C has limited serial positions: {sorted(set(serial.astype(int))) if not serial.empty else []}")

    d_df = panel_data.get("D")
    if d_df is not None:
        metrics = set(d_df.get("metric", pd.Series(dtype=str)).astype(str))
        if "rescued_fraction" in metrics:
            passes.append("Fig.3D uses rescued_fraction as the compact sequence-rescue summary")
        else:
            failures.append(f"Fig.3D missing rescued_fraction, found {sorted(metrics)}")
        if {"seq_len", "item_count"}.issubset(d_df.columns):
            passes.append("Fig.3D records K and rescued item count")
        else:
            failures.append("Fig.3D must record seq_len and item_count")

    qc_req = spec.get("qc_requirements") or {}
    full_k = {int(v) for v in (qc_req.get("expected_seq_lengths") or [3, 5, 7, 10])}
    full_delay = {int(v) for v in (qc_req.get("expected_delay_ms") or [100, 200, 400, 800])}
    expected_heatmap_metrics = {"E": "rescued_fraction"}
    for panel_id, expected_metric in expected_heatmap_metrics.items():
        df = panel_data.get(panel_id)
        if df is None:
            continue
        metrics = set(df.get("metric", pd.Series(dtype=str)).astype(str))
        if expected_metric in metrics:
            passes.append(f"Fig.3{panel_id} uses {expected_metric} delay-boundary heatmap")
        else:
            failures.append(f"Fig.3{panel_id} expected metric {expected_metric}, found {sorted(metrics)}")
        observed_k = {int(v) for v in pd.to_numeric(df.get("seq_len", pd.Series(dtype=float)), errors="coerce").dropna().unique()}
        observed_delay = {int(v) for v in pd.to_numeric(df.get("delay_ms", pd.Series(dtype=float)), errors="coerce").dropna().unique()}
        if observed_k == full_k:
            passes.append(f"Fig.3{panel_id}: full K grid {sorted(full_k)}")
        else:
            failures.append(f"Fig.3{panel_id}: K grid mismatch {sorted(observed_k)} != {sorted(full_k)}")
        if observed_delay == full_delay:
            passes.append(f"Fig.3{panel_id}: full delay grid {sorted(full_delay)}")
        else:
            failures.append(f"Fig.3{panel_id}: delay grid mismatch {sorted(observed_delay)} != {sorted(full_delay)}")

    if set(panels.keys()) == {"A", "B", "C", "D", "E"}:
        passes.append("Fig.3 main figure contains the target A-E two-row layout")
    else:
        failures.append(f"Fig.3 main figure must contain exactly A-E panels, found {sorted(panels.keys())}")
    axes = {pid: (panels.get(pid) or {}).get("axes_mm") or {} for pid in expected_order}
    if axes.get("C", {}).get("x") == axes.get("A", {}).get("x"):
        passes.append("Fig.3C y-axis aligns with Fig.3A")
    else:
        failures.append(f"Fig.3C y-axis must align with Fig.3A, found A={axes.get('A')} C={axes.get('C')}")
    b_right = float(axes.get("B", {}).get("x", 0)) + float(axes.get("B", {}).get("w", 0))
    e_right = float(axes.get("E", {}).get("x", 0)) + float(axes.get("E", {}).get("w", 0))
    if abs(b_right - e_right) <= 0.05:
        passes.append("Fig.3E x-axis right edge aligns with Fig.3B")
    else:
        failures.append(f"Fig.3E x-axis right edge must align with Fig.3B, found B_right={b_right}, E_right={e_right}")
    lower_widths = [float((axes.get(pid) or {}).get("w", 0)) for pid in ("C", "D", "E")]
    if max(lower_widths) - min(lower_widths) <= 0.05:
        passes.append("Fig.3C/D/E plotting axes have equal widths")
    else:
        failures.append(f"Fig.3C/D/E plotting axes must have equal widths, found {dict(zip(('C', 'D', 'E'), lower_widths))}")
    if render_metadata:
        clipped = {
            panel_id: list(meta.get("clipped_artists", []))
            for panel_id, meta in render_metadata.items()
...
def _check_fig3_delay_supp_contract(
    spec: Mapping[str, Any],
    panels: Mapping[str, Any],
    output_dir: Path,
    render_metadata: Mapping[str, Mapping[str, Any]],
    ...
    figure_id = "fig3_delay_supp"
    canvas = spec.get("canvas_mm") or {}
    if float(canvas.get("width", 0)) == 165 and float(canvas.get("height", 0)) == 82:
        passes.append("Fig.3 delay supplement canvas is 165 x 82 mm")
    else:
        failures.append(f"Fig.3 delay supplement canvas must be 165 x 82 mm, found {canvas}")
    expected = ["A", "B"]
    if list(spec.get("reading_order") or []) == expected and set(panels.keys()) == set(expected):
        passes.append("Fig.3 delay supplement has two-panel A/B heatmap structure")
    else:
        failures.append(f"Fig.3 delay supplement panels/reading_order must be {expected}, found panels={sorted(panels.keys())}, reading_order={spec.get('reading_order')}")
    expected_metrics = {"A": "N_eff_fraction", "B": "rescued_fraction"}
    expected_adapter = {"A": "fig3_delay_morphology_heatmap_adapter", "B": "fig3_delay_rescue_heatmap_adapter"}
    expected_renderers = {"A": "render_fig3_boundary_heatmap", "B": "render_fig3_boundary_heatmap"}
    qc_req = spec.get("qc_requirements") or {}
    full_k = {int(v) for v in (qc_req.get("expected_seq_lengths") or [3, 5, 7, 10])}
    full_delay = {int(v) for v in (qc_req.get("expected_delay_ms") or [100, 200, 400, 800])}
    for panel_id in expected:
        panel = panels.get(panel_id) or {}
        if panel.get("renderer") == expected_renderers[panel_id]:
            passes.append(f"Fig.3 delay supplement {panel_id}: renderer={expected_renderers[panel_id]}")
        else:
            failures.append(f"Fig.3 delay supplement {panel_id}: expected renderer {expected_renderers[panel_id]}, found {panel.get('renderer')}")
        if panel.get("data_adapter") == expected_adapter[panel_id]:
            passes.append(f"Fig.3 delay supplement {panel_id}: adapter={expected_adapter[panel_id]}")
        else:
            failures.append(f"Fig.3 delay supplement {panel_id}: expected adapter {expected_adapter[panel_id]}, found {panel.get('data_adapter')}")
        paths = panel_output_paths(output_dir, figure_id, panel_id)
        if not paths["panel_data"].exists() or not paths["stats"].exists() or not paths["sources"].exists():
            failures.append(f"Fig.3 delay supplement {panel_id}: missing adapter outputs")
            continue
        df = pd.read_csv(paths["panel_data"])
        stats = read_json(paths["stats"])
        sources = read_json(paths["sources"])
        if df.get("metric", pd.Series(dtype=str)).astype(str).eq("missing_source").any() or sources.get("status") == "missing_source":
            failures.append(f"Fig.3 delay supplement {panel_id}: missing source data")
            continue
        source_text = _panel_sources(output_dir, figure_id, panel_id)
        if "data/intermediates" in source_text or "data\\intermediates" in source_text:
            failures.append(f"Fig.3 delay supplement {panel_id}: plotting source manifest must not read data/intermediates")
        else:
            passes.append(f"Fig.3 delay supplement {panel_id}: plot-only sources avoid data/intermediates")
        metrics = set(df.get("metric", pd.Series(dtype=str)).astype(str))
        if expected_metrics[panel_id] in metrics:
            passes.append(f"Fig.3 delay supplement {panel_id}: uses {expected_metrics[panel_id]}")
        else:
            failures.append(f"Fig.3 delay supplement {panel_id}: expected metric {expected_metrics[panel_id]}, found {sorted(metrics)}")
        observed_k = {int(v) for v in pd.to_numeric(df.get("seq_len", pd.Series(dtype=float)), errors="coerce").dropna().unique()}
        observed_delay = {int(v) for v in pd.to_numeric(df.get("delay_ms", pd.Series(dtype=float)), errors="coerce").dropna().unique()}
        if observed_k == full_k:
            passes.append(f"Fig.3 delay supplement {panel_id}: full K grid {sorted(full_k)}")
        else:
            failures.append(f"Fig.3 delay supplement {panel_id}: K grid mismatch {sorted(observed_k)} != {sorted(full_k)}")
        if observed_delay == full_delay:
            passes.append(f"Fig.3 delay supplement {panel_id}: full delay grid {sorted(full_delay)}")
        else:
            failures.append(f"Fig.3 delay supplement {panel_id}: delay grid mismatch {sorted(observed_delay)} != {sorted(full_delay)}")
        run_mode = str(stats.get("run_mode") or sources.get("run_mode") or "")
        n_networks = int(stats.get("n_networks") or sources.get("n_networks") or 0)
        if run_mode == "single_network_draft" or n_networks == 1:
            warnings.append(f"Fig.3 delay supplement {panel_id}: single_network_draft n_networks=1; draft-only, not final manuscript statistics")
    if render_metadata:
        placeholders = [pid for pid, meta in render_metadata.items() if meta.get("plot_form") == "blank_panel" or meta.get("placeholder_reason")]
        if placeholders:
            failures.append(f"Fig.3 delay supplement must not have blank/placeholder renderers: {placeholders}")
        else:
            passes.append("Fig.3 delay supplement rendered without blank placeholders")
...
def _check_fig3_specifics(
    ...
    if figure_id == "fig3_supp":
        ...
        return
    if figure_id == "fig3_delay_supp":
        _check_fig3_delay_supp_contract(spec, panels, output_dir, render_metadata, passes, warnings, failures)
        return
    if figure_id != "fig3":
        return
...
def _check_fig5_specifics(
    ...
    expected_height = float(expected_canvas.get("height", 118))
    if float(canvas.get("width", 0)) == expected_width and float(canvas.get("height", 0)) == expected_height:
        passes.append(f"Fig.5 canvas is {expected_width:.0f} x {expected_height:.0f} mm")
    else:
        failures.append(f"Fig.5 canvas must be {expected_width:.0f} x {expected_height:.0f} mm, found {canvas}")
    _check_fig5_geometry(spec, panels, render_metadata, passes, failures)

    panel_data: dict[str, pd.DataFrame] = {}
    stats_by_panel: dict[str, Mapping[str, Any]] = {}
    sources_by_panel: dict[str, Mapping[str, Any]] = {}
    for panel_id in ("A", "B", "C", "D", "E"):
        if panel_id not in panels:
    ...
    if panel_data:
        ...
        for df in panel_data.values():
            ...
            conditions.update(str(v) for v in df.get("reference_condition", pd.Series(dtype=str)).dropna().unique())
            conditions.update(str(v) for v in df.get("comparison_condition", pd.Series(dtype=str)).dropna().unique())
        ...
        required_panel_metrics = {
            ...
            "D": {"l2_reupdate_probability_given_history"},
            "E": {"P_advance_or_recruit_dynamic_minus_condition"},
        }
        ...
        if d_df is not None:
            ...
            d_history = set(str(v) for v in d_df.get("history_status", pd.Series(dtype=str)).dropna().unique())
            ...
            expected_history = {"prior_updated", "not_prior_updated"}
            if d_metrics == {"l2_reupdate_probability_given_history"}:
                passes.append("Fig.5D uses Layer2 re-update probability by history")
            else:
                failures.append(f"Fig.5D must use l2_reupdate_probability_given_history only, found {sorted(d_metrics)}")
            if {"dynamic_intact", "static_opportunity"}.issubset(d_conditions) and {"Static", "Dynamic"}.issubset(d_labels):
                passes.append("Fig.5D compares Dynamic writeback against Static opportunity")
            else:
                failures.append(f"Fig.5D must include Dynamic/Static labels and dynamic_intact/static_opportunity conditions, found labels={sorted(d_labels)} conditions={sorted(d_conditions)}")
            if d_history == expected_history:
                passes.append("Fig.5D includes prior-updated and not-prior-updated history segments")
            else:
                failures.append(f"Fig.5D must include exactly prior/not-prior history segments, found {sorted(d_history)}")
            if "panel_postprobe_l2_reupdate_history_composition.csv" in source_text_d:
                passes.append("Fig.5D source is Layer2 re-update history composition")
            else:
                failures.append("Fig.5D must use panel_postprobe_l2_reupdate_history_composition.csv")
            if _fig5_prior_exceeds_nonprior_by_condition(d_df):
                passes.append("Fig.5D prior-updated sites exceed not-prior-updated sites in each condition")
            else:
                failures.append("Fig.5D expected prior-updated sites to exceed not-prior-updated sites in Dynamic and Static")

        e_df = panel_data.get("E")
        if e_df is not None:
            e_stats = stats_by_panel.get("E", {})
            e_reference_conditions = set(str(v) for v in e_df.get("reference_condition", pd.Series(dtype=str)).dropna().unique())
            e_comparison_conditions = set(str(v) for v in e_df.get("comparison_condition", pd.Series(dtype=str)).dropna().unique())
            expected_comparisons = {"attenuate_l1_stsp", "reset_l1_stsp"}
            if e_reference_conditions == {"dynamic_intact"} and e_comparison_conditions == expected_comparisons:
                passes.append("Fig.5E uses dynamic as the paired reference against Layer1-attenuate and Layer1-reset conditions")
            else:
                failures.append(
                    "Fig.5E must use exactly dynamic_intact as the reference and attenuate_l1_stsp/reset_l1_stsp as comparisons, "
                    f"found references={sorted(e_reference_conditions)} comparisons={sorted(e_comparison_conditions)}"
                )
            source_text_e = " ".join(str(v) for v in e_df.get("source_file", pd.Series(dtype=str)).dropna().unique()).lower()
            e_metrics = set(str(v) for v in e_df.get("metric", pd.Series(dtype=str)).dropna().unique())
            headline_metric = "P_advance_or_recruit_dynamic_minus_condition"
            if e_metrics == {headline_metric}:
                passes.append("Fig.5E uses only the frozen advance-or-recruit paired headline metric")
            else:
                failures.append(f"Fig.5E must use only {headline_metric}, found metrics={sorted(e_metrics)}")
            excluded_transitions = set(str(v) for v in e_df.get("transition_types_excluded", pd.Series(dtype=str)).dropna().unique())
            included_transitions = set(str(v) for v in e_df.get("transition_types_included", pd.Series(dtype=str)).dropna().unique())
            if included_transitions == {"advance;recruit"} and excluded_transitions == {"loss"}:
                passes.append("Fig.5E headline includes advance/recruit and explicitly excludes loss")
            else:
                failures.append(
                    "Fig.5E headline must include advance;recruit and exclude loss, "
                    f"found included={sorted(included_transitions)} excluded={sorted(excluded_transitions)}"
                )
            e_labels = set(str(v) for v in e_df.get("condition", pd.Series(dtype=str)).dropna().unique())
            expected_labels = {"Dynamic - Attenuate L1 STSP", "Dynamic - Reset L1 STSP"}
            if e_labels == expected_labels:
                passes.append("Fig.5E labels the two frozen dynamic-minus-condition contrasts")
            else:
                failures.append(f"Fig.5E must label the two dynamic-minus-condition contrasts, found labels={sorted(e_labels)}")
            network_counts = {
                comparison: int(part.get("network_id", pd.Series(dtype=str)).astype(str).nunique())
                for comparison, part in e_df.groupby("comparison_condition", dropna=False)
            }
            if network_counts == {comparison: 20 for comparison in expected_comparisons}:
                passes.append("Fig.5E contains 20 independent network contrasts for each paired comparison")
            else:
                failures.append(f"Fig.5E must contain 20 network contrasts per comparison, found {network_counts}")
            ci_labels = set(str(v) for v in e_df.get("confidence_interval", pd.Series(dtype=str)).dropna().unique())
            if ci_labels == {"two-sided paired t-based 95% CI"}:
                passes.append("Fig.5E records paired t-based 95% CI provenance")
            else:
                failures.append(f"Fig.5E must record paired t-based 95% CI provenance, found {sorted(ci_labels)}")
            summaries = e_stats.get("contrast_summaries") or []
            summary_by_comparison = {
                str(item.get("comparison_condition")): item
                for item in summaries
                if isinstance(item, Mapping)
            }
            if all(
                int(summary_by_comparison.get(comparison, {}).get("n_networks", 0) or 0) == 20
                and {"ci95_lower", "ci95_upper", "p_value_two_sided"}.issubset(summary_by_comparison.get(comparison, {}))
                for comparison in expected_comparisons
            ):
                passes.append("Fig.5E stats contain paired t-based 95% CI and p value for both 20-network contrasts")
            else:
                failures.append("Fig.5E stats must contain both 20-network paired t-based CI summaries")
            forbidden_e_metrics = {"dynamic_like_spike_similarity", "decision_deflection_score", "loser_post_winner_inh_rise", "early_recruitment", "spike_similarity", "decision_deflection"}
            if forbidden_e_metrics.isdisjoint(e_metrics) and "panel_d_support_perturbation_node_metrics.csv" not in source_text_e:
                passes.append("Fig.5E does not use node similarity/decision summary as the main visual")
            else:
                failures.append(f"Fig.5E must not use node similarity/decision summary, found metrics={sorted(forbidden_e_metrics.intersection(e_metrics))}")
            if "panel_d_l1_stsp_perturbation_unit_transitions.csv" in source_text_e:
                passes.append("Fig.5E source is Layer1 STSP unit transitions")
                windows = set(float(v) for v in pd.to_numeric(e_df.get("analysis_window_ms", pd.Series(dtype=float)), errors="coerce").dropna().unique())
                if 50.0 in windows:
                    passes.append("Fig.5E uses first-50-ms transition analysis window")
                else:
                    failures.append(f"Fig.5E unit-transition source must record analysis_window_ms=50, found {sorted(windows)}")
            elif "panel_d_l1_stsp_perturbation_transition_summary.csv" in source_text_e:
                passes.append("Fig.5E source is Layer1 STSP perturbation transition summary")
            else:
                failures.append("Fig.5E must use panel_d_l1_stsp_perturbation_unit_transitions.csv or panel_d_l1_stsp_perturbation_transition_summary.csv")
        ...
        c_meta = render_metadata.get("C") or {}
        c_trace_metrics = set(str(metric) for metric in (c_meta.get("trace_metrics") or []))
        if not c_trace_metrics:
            c_trace_metrics = set(str(metric) for metric in ((panels.get("C") or {}).get("metrics") or []))
        if c_trace_metrics == {"winner_delta_v", "loser_delta_v"}:
            passes.append("Fig.5C renders only winner and loser voltage traces")
        else:
            failures.append(f"Fig.5C must render only winner_delta_v and loser_delta_v, found {sorted(c_trace_metrics)}")
        if "Inhibition received by loser" in set(str(text) for text in (c_meta.get("legend_texts") or [])):
            failures.append("Fig.5C legend must not include the inhibition trace")
        else:
            passes.append("Fig.5C legend excludes the inhibition trace")
        for subtitle_panel in ("D", "E"):
            title_spec = str((panels.get(subtitle_panel) or {}).get("title", "")).strip()
            title_rendered = str((render_metadata.get(subtitle_panel) or {}).get("title", "")).strip()
            if not title_spec and not title_rendered:
                passes.append(f"Fig.5{subtitle_panel} has no small panel subtitle")
            else:
                failures.append(f"Fig.5{subtitle_panel} subtitle must be removed, found spec={title_spec!r}, rendered={title_rendered!r}")
    ...
    if render_metadata:
        ...
        e_meta = render_metadata.get("E") or {}
        if (
            e_meta.get("plot_form") == "fig5_l1_stsp_advance_or_recruit_paired_contrast"
            and bool(e_meta.get("raw_points"))
            and int(e_meta.get("raw_point_count", 0) or 0) == 40
            and e_meta.get("y_metric") == "P_advance_or_recruit_dynamic_minus_condition"
        ):
            passes.append("Fig.5E renders all 40 network-level paired contrasts with the frozen headline metric")
        else:
            failures.append("Fig.5E must render 40 network-level paired contrasts for the frozen advance-or-recruit headline metric")
    return
...
def _check_fig5_geometry(spec: Mapping[str, Any], panels: Mapping[str, Any], render_metadata: Mapping[str, Mapping[str, Any]], passes: list[str], failures: list[str]) -> None:
    ids = ("A", "B", "C", "D", "E")
    ...
    if any(not pos[panel_id] for panel_id in ids):
        ...
        return
    expected = {
        "A": {"x": 2.00, "y": 10.00, "w": 79.50, "h": 48.00},
        "B": {"x": 83.50, "y": 10.00, "w": 79.50, "h": 48.00},
        "C": {"x": 2.00, "y": 60.00, "w": 52.333, "h": 48.00},
        "D": {"x": 56.333, "y": 60.00, "w": 52.333, "h": 48.00},
        "E": {"x": 110.667, "y": 60.00, "w": 52.333, "h": 48.00},
    }
    for panel_id, expected_pos in expected.items():
        if _fig5_box_near(pos[panel_id], expected_pos, tol=0.10):
            passes.append(f"Fig.5{panel_id} position matches requested mm layout")
        else:
    ...
    if all(_near(_y(pos["C"]), _y(pos[pid]), tol=0.15) and _near(_bottom(pos["C"]), _bottom(pos[pid]), tol=0.15) for pid in ("D", "E")):
        passes.append("Fig.5 C/D/E form the bottom row")
    else:
        failures.append("Fig.5 C/D/E must form the bottom row")
    if _near(_x(pos["A"]), _x(pos["C"]), tol=0.15):
        passes.append("Fig.5 bottom-left C y-axis aligns with top-left A")
    else:
        failures.append("Fig.5 C must align with A on the left edge")
    if _near(_right(pos["B"]), _right(pos["E"]), tol=0.15):
        passes.append("Fig.5 bottom-right E aligns with top-right B")
    else:
        failures.append("Fig.5 E right edge must align with B right edge")
    row_gap = _y(pos["C"]) - _bottom(pos["A"])
    col_gap_top = _x(pos["B"]) - _right(pos["A"])
    col_gap_bottom_cd = _x(pos["D"]) - _right(pos["C"])
    col_gap_bottom_de = _x(pos["E"]) - _right(pos["D"])
    if _near(row_gap, 2.0, tol=0.15) and _near(col_gap_top, 2.0, tol=0.15) and _near(col_gap_bottom_cd, 2.0, tol=0.15) and _near(col_gap_bottom_de, 2.0, tol=0.15):
        passes.append("Fig.5 row and column gaps are clean and positive")
    else:
        failures.append(f"Fig.5 gaps must be 2 mm: row={row_gap:.2f}, top_col={col_gap_top:.2f}, bottom_cols=({col_gap_bottom_cd:.2f},{col_gap_bottom_de:.2f})")
    left_margin = min(_x(pos[pid]) for pid in ids)
    top_margin = min(_y(pos[pid]) for pid in ids)
    canvas = spec.get("canvas_mm") or {}
    canvas_w = float(canvas.get("width", 165.0))
    canvas_h = float(canvas.get("height", 110.0))
    right_margin = canvas_w - max(_right(pos[pid]) for pid in ids)
    bottom_margin = canvas_h - max(_bottom(pos[pid]) for pid in ids)
    if _near(left_margin, 2.0, tol=0.15) and _near(top_margin, 10.0, tol=0.15) and _near(right_margin, 2.0, tol=0.15) and _near(bottom_margin, 2.0, tol=0.15):
        passes.append("Fig.5 uses stable outer margins on all sides")
    else:
        failures.append(f"Fig.5 outer margins must be left/right 2 mm, top 10 mm, and bottom 2 mm: left={left_margin:.2f}, top={top_margin:.2f}, right={right_margin:.2f}, bottom={bottom_margin:.2f}")
    ...
    if all(isinstance(axes[pid], list) and len(axes[pid]) == 4 for pid in ids):
        ...
        if _near(axes["A"][0], axes["C"][0], tol=0.004) and _near(axes["B"][2], axes["E"][2], tol=0.004):
            passes.append("Fig.5 rendered plotting axes preserve required left/right alignment")
        else:
            passes.append("Fig.5 raw plotting axes differ after true-edge text fitting; full-panel bboxes carry alignment")
        if axes["C"][2] < axes["D"][0] and axes["D"][2] < axes["E"][0]:
            passes.append("Fig.5 rendered bottom-row axes do not overlap")
        else:
            failures.append("Fig.5 rendered C/D/E axes must not overlap")


def _fig5_dynamic_exceeds_static(df: pd.DataFrame) -> bool:
    if df.empty or not {"condition", "value"}.issubset(df.columns):
        return False
    if {"history_status", "condition"}.issubset(df.columns):
        part = df[df["history_status"].astype(str).eq("prior_updated")]
        ...
        return not dynamic.empty and not static.empty and float(dynamic.mean()) > float(static.mean())
    if "unit_group" not in df.columns:
        return False
    expected_groups = ["overlap_dominant", "probe_only_dominant", "random_matched"]
    for group in expected_groups:
        part = df[df["unit_group"].astype(str).eq(group)]
        static = pd.to_numeric(part.loc[part["condition"].astype(str).eq("Static"), "value"], errors="coerce").dropna()
        dynamic = pd.to_numeric(part.loc[part["condition"].astype(str).eq("Dynamic"), "value"], errors="coerce").dropna()
        if static.empty or dynamic.empty or not float(dynamic.mean()) > float(static.mean()):
            return False
    return True


def _fig5_prior_exceeds_nonprior_by_condition(df: pd.DataFrame) -> bool:
    if df.empty or not {"condition", "history_status", "value"}.issubset(df.columns):
        return False
    for condition in ("Dynamic", "Static"):
        ...
        if prior.empty or nonprior.empty or not float(prior.mean()) > float(nonprior.mean()):
            return False
    return True
...
def _check_fig6_stsp_recruitment_contract(
    ...
    if float(canvas.get("width", 0)) == 165 and float(canvas.get("height", 0)) == 102:
        ...
        failures.append(f"Fig.6 canvas must be 165 x 102 mm for the A-E overlap-gated STSP layout, found {canvas}")

    expected_panels = {"A", "B", "C", "D", "E"}
    if set(panels.keys()) == expected_panels:
        passes.append("Fig.6 panels are A-E")
    else:
        failures.append(f"Fig.6 must contain exactly panels A-E, found {sorted(panels.keys())}")
    if list(spec.get("reading_order") or []) == ["A", "B", "C", "D", "E"]:
        passes.append("Fig.6 reading order is A-E")
    else:
        failures.append(f"Fig.6 reading_order must be A-E, found {spec.get('reading_order')}")

    expected_boxes = {
        "A": {"x": 2.00, "y": 2.00, "w": 52.333, "h": 48.00},
        "B": {"x": 56.333, "y": 2.00, "w": 52.333, "h": 48.00},
        "C": {"x": 110.667, "y": 2.00, "w": 52.333, "h": 48.00},
        "D": {"x": 2.00, "y": 52.00, "w": 79.50, "h": 48.00},
        "E": {"x": 83.50, "y": 52.00, "w": 79.50, "h": 48.00},
    }
    ...
    if {"old_mass", "middle_mass", "recent_mass"}.issubset(a_metrics) and {"Peak ping", "Valley ping", "Random ping"}.issubset(a_conditions):
        passes.append("Fig.6A contains peak/valley/random old/middle/recent readout mass metrics")
    else:
        failures.append("Fig.6A must contain old/middle/recent mass for Peak/Valley/Random ping")

    if "other_mass" in a_metrics:
        passes.append("Fig.6A source data include optional other_mass while the main renderer omits that stack segment")
    else:
        passes.append("Fig.6A optional other_mass is absent from source data")
    visible_a_metrics = set(str(metric) for metric in ((render_metadata.get("A") or {}).get("stack_metrics") or ((panels.get("A") or {}).get("metrics") or [])))
    if visible_a_metrics == {"old_mass", "recent_mass", "middle_mass"}:
        passes.append("Fig.6A visible stack is limited to old, recent, and middle")
    else:
        failures.append(f"Fig.6A visible stack must contain only old/recent/middle, found {sorted(visible_a_metrics)}")
    ...
    if render_metadata:
        ...
        if (render_metadata.get("B") or {}).get("x_label") == "STSP score quantile" and str((render_metadata.get("B") or {}).get("y_label", "")) == "Layer 1 spike probability (%)":
            passes.append("Fig.6B axes are score quantile versus Layer 1 spike probability")
        else:
            failures.append("Fig.6B axes must be STSP score quantile versus Layer 1 spike probability (%)")
        if (render_metadata.get("C") or {}).get("x_label") == "STSP score quantile" and str((render_metadata.get("C") or {}).get("y_label", "")) == "Layer 1 firing change (%)":
        ...
        if "F" in render_metadata:
            ...
            passes.append("Fig.6F renderer metadata absent as expected")
...
def _check_fig4_standalone_contract(
    ...
    if _fig4_near(float(canvas.get("width", 0)), 165.0) and _fig4_near(float(canvas.get("height", 0)), 152.0):
        ...
        failures.append(f"Fig.4 canvas must be 165 x 152 mm, found {canvas}")
    if list(spec.get("reading_order") or []) == ["A", "B", "C", "D", "E", "F"]:
...
def _check_fig4_current_geometry(panels: Mapping[str, Any], passes: list[str], failures: list[str]) -> None:
    expected = {
        "A": {"x": 2.00, "y": 2.00, "w": 79.50, "h": 48.00},
        "B": {"x": 83.50, "y": 2.00, "w": 79.50, "h": 48.00},
        "C": {"x": 2.00, "y": 52.00, "w": 79.50, "h": 48.00},
        "D": {"x": 83.50, "y": 52.00, "w": 79.50, "h": 48.00},
        "E": {"x": 2.00, "y": 102.00, "w": 79.50, "h": 48.00},
        "F": {"x": 83.50, "y": 102.00, "w": 79.50, "h": 48.00},
    }
    for panel_id, expected_pos in expected.items():
        pos = (panels.get(panel_id) or {}).get("position_mm") or {}
    ...
    if all(_fig4_near(float(pos[p].get("y", 0)), 2.0) for p in ("A", "B")):
        passes.append("Fig.4A/B are aligned in row 1")
    if all(_fig4_near(float(pos[p].get("y", 0)), 52.0) for p in ("C", "D")):
        passes.append("Fig.4C/D are aligned in row 2")
    if all(_fig4_near(float(pos[p].get("y", 0)), 102.0) for p in ("E", "F")):
...
def _fig_target_box_near(actual: Mapping[str, Any], expected: Mapping[str, float], *, tol: float = 0.08) -> bool:
    return all(_near(float(actual.get(key, -999.0)), value, tol=tol) for key, value in expected.items())
