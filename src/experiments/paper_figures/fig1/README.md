...
- `artifacts.py` 作为 Fig.1 payload Adapter，读写并校验 trial specs、延迟特征和 DMS 边界状态 artifact。
- `cache_keys.py` 构造 Fig.1 persisted artifact 的领域 cache key 和内容摘要。
...
- `run_task.py` 按声明依赖执行 Fig.1 单项或 scope DAG task，并通过共用生命周期 Module 复用匹配的 persisted artifact。
- `run.py` 提供 Fig.1 批量运行入口。
- `schemas.py` 定义 Fig.1 artifact schema 和 task 标识，并复用统一的 artifact 模式合同。
