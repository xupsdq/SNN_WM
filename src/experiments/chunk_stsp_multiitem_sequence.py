...
from src.plotting.experiments.chunk_stsp_multiitem_sequence_plot_lib import (
...
def main() -> None:
    ...
    # Record the minimal on-disk contract required for plot-only figure replay.
    save_run_config(
        {
            "bundle_type": "chunk_stsp_multiitem_sequence_plot_inputs",
            "version": 1,
            "files": DEFAULT_PLOT_BUNDLE_FILES,
        },
        meta_dir,
        filename="plot_bundle_manifest.json",
    )
    exported_files["plot_bundle_manifest_json"] = str(meta_dir / "plot_bundle_manifest.json")
    save_run_config(
        {
